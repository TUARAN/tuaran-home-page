...
**动笔前先读该文件的 active 版本**，全文按其 `principles`（当前骨架、事实研判分离、表格优先、
先结论后展开）与 `howToApply`（frontmatter 分工、来源与持续验证、收口三件套、措辞 grep 黑名单）执行。
本文件不再复述规则内容，避免多处漂移；`research/README.md` 的骨架仅作最低保底，冲突时以 active 版本为准。
