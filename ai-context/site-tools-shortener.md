...
`/ai-projects` 页面在「AI Native 项目图谱」之外，新增了一个「站内工具」板块，定位是「自用的小工具集合」。第一个工具是 **转短**：把长链接通过 TinyURL 压成短链，并把记录持久化到 Cloudflare D1，按账号隔离。

种子链接 `http://112.124.37.151:48575/#/register?code=TYbpJVsl` 是该工具上线时第一条要转的链接，会在用户首次进入、列表为空时被预填到输入框中（不写库），点一次「转短」即落库。

## 2. 用户视角的能力

- 登录后才能使用（GitHub OAuth，与 stomp 留言、xiaomoli-dad-todo 共用同一套自定义 Edge 会话）。
- 在输入框粘贴 `http(s)://` 长链接，点「转短」→ 调用 TinyURL 拿到短链 → 落库 → 列表头部出现新记录。
...
| `app/components/SiteToolsPanel.jsx` | 「站内工具」板块的客户端组件，承载「转短」工具的全部 UI 与交互 |
| `app/api/short/route.js` | Edge 路由，提供 `GET / POST / DELETE`，统一鉴权 + 落 D1 |
...
- `short` 列 NOT NULL，写入前会先调用 TinyURL 拿到结果；上游失败不落库，避免脏数据。
- 索引覆盖列表查询 `WHERE user_id = ? ORDER BY created_at DESC LIMIT 100`。
...
2. `shortenViaTinyUrl`：调用 `https://tinyurl.com/api-create.php?url=...`；上游非 200 → `502 UPSTREAM_FAILED`，返回值非 `http(s)://` → `502 UPSTREAM_INVALID`。
3. `INSERT ... RETURNING ...` 写库并把新记录返回给前端，HTTP 201。

成功响应：`{ "item": { "id": ..., "original": ..., "short": ..., "created_at": ... } }`
...
- TinyURL 返回的短链不会保留 URL `#fragment`（`#` 部分本身不会发到任何服务器），如果将来要让带哈希路由的链接也可以「整段还原」，需要换成自建短链表（在 `short_links` 上加 `code` 列 + `/s/:code` 路由）。
- 当前没有提供翻页与搜索，只取最近 100 条；如果记录密度上来了，再加 `?cursor=` 或全文搜索。
- 没有针对短链做有效性回查，TinyURL 端如果失效，本地记录不会自动清理。

## 9. 后续可扩展方向

- 把「站内工具」做成真正的工具集合：第二个工具可能是「Markdown → 朋友圈分段」「URL 二维码」「站内反链查询」等等，复用 `SiteToolsPanel` 的卡片样式。
- 支持自建短链：新增 `code TEXT UNIQUE`，写入时随机生成，提供 `/s/:code` 重定向路由，彻底脱离 TinyURL 依赖；同时也能保留 fragment。
