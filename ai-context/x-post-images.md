...
- 每天保留 10 个自动时间节点：08:00 问候；09:00、15:00 朋友交流；10:00、20:00 文化短故事；11:00、17:00 加密观点；23:00、次日 03:00、次日 07:00 美区英文。每个时段按上海日期计算前后 30 分钟内的随机计划时间；同一天重试沿用原计划，跨日重新计算。午安、晚安、下午文化、晚间交友、加密人物五个旧时段不再进入自动调度，历史记录和手动接口兼容仍保留。
...
- 问候、朋友图文、文化短故事、加密观点、美区英文共 10 个自动时段中的图文任务：尚无已保存配图时，用随机数等概率选择两种策略。小于 0.5 时生成优先，失败或超过 90 秒则随机使用同主题 R2 素材兜底；大于等于 0.5 时直接随机选用同主题、已启用的备用素材，不调用提示词生成或生图服务，也不混入历史在线生成图。直接素材分支遇到无可用素材时返回失败，不反向启动生图；任一分支没有图片或上传失败时均不发送纯文字。提示词生成另有 45 秒超时。超时不代表供应商已取消计算，费用仍以供应商账单为准。
...
5. 观察自然调度：核对执行时间和发布形式，记录变为已发布；图文有图，纯文本无图。不要为验收额外触发所有 10 个时段。
