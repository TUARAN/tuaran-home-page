...
- 问候、朋友图文、文化短故事、加密观点、美区英文共 15 个时段：尚无已保存配图时，用随机数等概率选择两种策略。小于 0.5 时生成优先，失败或超过 90 秒则随机使用同主题 R2 素材兜底；大于等于 0.5 时直接随机选用同主题、已启用的备用素材，不调用提示词生成或生图服务，也不混入历史在线生成图。直接素材分支遇到无可用素材时返回失败，不反向启动生图；任一分支没有图片或上传失败时均不发送纯文字。提示词生成另有 45 秒超时。超时不代表供应商已取消计算，费用仍以供应商账单为准。
...
先保存文案，再随机选择生成优先或直接素材，保存图片对象键后上传并发布到 X。上传失败重试时优先复用已保存的文案、提示词和图片，不重新抽策略，重新获取 X media ID，不重复生图或换图；已保存对象丢失时返回错误，不静默换图。尚未保存图片时的重试会重新抽取策略。记录复用现有 `asset_source`、`pool_asset_id` 和 `fallback_error`，无需数据库迁移；直接选素材时 `fallback_error` 为空，生成失败兜底时保存实际失败原因，后台分别显示来源。D1 不可用时停止发布，避免绕过暂停和去重。
