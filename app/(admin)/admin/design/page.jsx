...
const documents = [
  { id: 'audit', title: 'UI / 交互审计与改造清单', source: 'ai-context/ui-ux-audit-roadmap.md', markdown: audit },
...
function DocumentSection({ document }) {
  ...
  return (
    <div id={document.id} className="scroll-mt-24">
      <Section title={document.title} description={`记录来源：${document.source}`}>
...
export default function AdminDesignPage() {
  const tasks = [...audit.matchAll(/^- \[([ xX])\] UI-\d+：/gm)]
    ...
    <AdminPageGate label="设计与体验" returnTo="/admin/design">
      ...
      >
        <div className="grid gap-3 sm:grid-cols-3">
          <StatCard label="归档任务进度" value={`${completed}/${tasks.length}`} sub="以审计文档的勾选记录为准" icon="planning" tone="info" />
        ...
        <p className="mb-5 text-sm leading-7 text-[var(--admin-muted)]">
          先修手机长文目录、动态内容失败提示，以及后台概览和设计令牌。清单用于阅读与验收，修改文档后随部署同步；具体执行进入“规划与待办”，当前尚未自动同步任务。
        </p>
