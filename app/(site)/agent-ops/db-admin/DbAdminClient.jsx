    ...
    <main className="mx-auto w-full max-w-[1180px] px-4 py-8 md:py-12">
      <header className="mb-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mb-0 max-w-[46rem] text-[14px] leading-7 text-[#5d554a] dark:text-gray-300">
      ...
      {snapshot?.summaries?.length ? (
        <section className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          {snapshot.summaries.map((item) => (
            <div key={item.key} className="rounded-xl border border-[#e5dccd] bg-white/70 px-4 py-3 dark:border-[#252e39] dark:bg-[#10161f]">
              <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9c8f79] dark:text-[#8e9ab0]">
                {item.label}
              </div>
              <div className="mt-1 text-2xl font-semibold text-[#221f19] dark:text-gray-100">
                {formatNumber(item.value)} <span className="text-xs font-normal text-[#75695a] dark:text-[#9aa6b6]">{item.unit}</span>
              </div>
              <div className="mt-1 text-[11px] text-[#75695a] dark:text-[#9aa6b6]">{item.hint}</div>
            </div>
          ))}
        </section>
      ) : null}
      ...
      <section className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <input
          ...
          className="w-full rounded-lg border border-[#dcd3c0] bg-white px-3 py-1.5 text-sm outline-none focus:border-[#a37b3c] dark:border-[#2d3744] dark:bg-[#10161f] dark:text-gray-100 sm:w-80"
        />
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="overflow-hidden rounded-xl border border-[#e5dccd] dark:border-[#252e39]">
          <table className="w-full border-collapse text-left text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-center text-sm text-[#9c8f79] dark:text-[#8e9ab0]">
                    加载中…
                  </td>
                </tr>
                ...
                filteredTables.map((table) => (
                  <tr
                    ...
                    className={`cursor-pointer border-t border-[#ece5d8] transition hover:bg-[#fbf7ef] dark:border-[#252e39] dark:hover:bg-[#151c25] ${
                      selected?.name === table.name ? 'bg-[#fff7ed] dark:bg-[#2a2113]' : ''
                    }`}
                  >
                    <td className="px-3 py-3 align-top">
                      <div className="flex flex-wrap items-center gap-2">
                        <b className="text-[#221f19] dark:text-gray-100">{table.label}</b>
                      ...
                      <div className="mt-1 font-mono text-[11px] text-[#9c8f79] dark:text-[#8e9ab0]">{table.name}</div>
                      <div className="mt-1 text-[12px] text-[#75695a] dark:text-[#9aa6b6]">{table.description}</div>
                    </td>
                    <td className="px-3 py-3 align-top font-mono text-sm text-[#221f19] dark:text-gray-100">
                    ...
                    <td className="px-3 py-3 align-top text-[12px] text-[#75695a] dark:text-[#9aa6b6]">
                      ...
                      <div className="font-mono text-[10px] text-[#9c8f79] dark:text-[#8e9ab0]">{table.recentColumn || 'no time column'}</div>
                    </td>
                    <td className="px-3 py-3 align-top font-mono text-xs text-[#75695a] dark:text-[#9aa6b6]">
        ...
        <aside className="rounded-xl border border-[#e5dccd] bg-white/70 p-4 dark:border-[#252e39] dark:bg-[#10161f]">
          {selected ? (
            <>
              <div className="mb-4">
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9c8f79] dark:text-[#8e9ab0]">
                  Table Detail
                </p>
                <h2 className="mt-1 break-words text-lg font-semibold text-[#221f19] dark:text-gray-100">{selected.name}</h2>
                <p className="mt-1 text-sm leading-6 text-[#75695a] dark:text-[#9aa6b6]">{selected.description}</p>
              </div>
              <dl className="mb-4 grid grid-cols-[72px_minmax(0,1fr)] gap-x-3 gap-y-2 text-sm">
                <dt className="text-[#9c8f79] dark:text-[#8e9ab0]">分组</dt><dd>{selected.group}</dd>
                <dt className="text-[#9c8f79] dark:text-[#8e9ab0]">行数</dt><dd>{formatNumber(selected.rowCount)}</dd>
                <dt className="text-[#9c8f79] dark:text-[#8e9ab0]">字段</dt><dd>{formatNumber(selected.columnsCount)}</dd>
                <dt className="text-[#9c8f79] dark:text-[#8e9ab0]">索引</dt><dd>{formatNumber(selected.indexesCount)}</dd>
                <dt className="text-[#9c8f79] dark:text-[#8e9ab0]">文本量</dt><dd>{formatBytes(selected.approxTextBytes)}</dd>
              </dl>
              <div className="overflow-hidden rounded-lg border border-[#ece5d8] dark:border-[#252e39]">
                <table className="w-full border-collapse text-left text-[12px]">
                  <thead className="bg-[#f8f3e7] text-[#7a6c54] dark:bg-[#151c25] dark:text-[#8e9ab0]">
                    <tr>
                  ...
                  <tbody>
                    {(selected.columns || []).map((column) => (
                      <tr key={column.name} className="border-t border-[#ece5d8] dark:border-[#252e39]">
                        <td className="break-all px-2 py-1.5 font-mono">{column.name}</td>
                        <td className="px-2 py-1.5 text-[#75695a] dark:text-[#9aa6b6]">{column.type || '—'}</td>
                        <td className="px-2 py-1.5 text-[#75695a] dark:text-[#9aa6b6]">
            ...
            <p className="text-sm text-[#75695a] dark:text-[#9aa6b6]">选择一张表查看字段。</p>
          )}
        </aside>
      </section>
    </main>
...
function Stat({ label, value, hint, toneClass, compact = false }) {
  return (
    <div className={`rounded-xl border px-4 py-3 ${toneClass || 'border-[#e5dccd] bg-white/70 dark:border-[#252e39] dark:bg-[#10161f]'}`}>
...
function ListNotice({ title, items, empty }) {
  return (
    <div className="rounded-xl border border-[#e5dccd] bg-white/70 px-4 py-3 text-sm dark:border-[#252e39] dark:bg-[#10161f]">
      <b className="block text-[#221f19] dark:text-gray-100">{title}</b>
      <p className="mt-1 break-words font-mono text-[12px] text-[#75695a] dark:text-[#9aa6b6]">
