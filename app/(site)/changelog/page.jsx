...
const changelog = [
  ...
  },
  {
    ...
    commits: 33,
    title: '账号身份中心、前端周刊与资源交付能力上线',
    summary:
      '本轮把“内容、账号、权益”三条原本分散的路径继续连起来：前台新增前端周刊和 AI 音乐资源入口，账号页升级为可管理多平台身份的个人中心，燃币与资源领取规则回到统一配置；同时补齐私密内容、资产管理和财务记录等站长侧能力，让公开内容与长期运营都更有承接。',
    planned: [
      '在真实 D1、邮件与第三方 OAuth 环境完成账号绑定、待激活账号和资源领取的端到端回归，重点核对重复身份与权益迁移边界。',
      '继续补全前端周刊的历史日更数据、内容筛选与移动端阅读细节，并观察定时同步的稳定性。',
      '梳理私密内容中心、财务记录与分享页之间的权限边界，补齐最小化的审计与异常空态。',
    ],
    done: [
      '新增 /frontend-weekly 前端周刊：支持周刊索引、日更条目、实时流和站内导航入口，并配置日更、实时与周刊同步工作流。',
      '账号中心升级为平台身份中心：集中展示邮箱、GitHub、Google、微信等登录身份，支持待绑定微信登录与账号关联，并简化登录后的身份管理流程。',
      '重构燃币与资源权益：统一资源解锁与领取逻辑，把积分规则、管理员配置和资源交付接口收束到可维护的配置层。',
      '新增私密 NSFW 媒体库后台、加密图片分享与 Long Compass 财务视图；私有内容继续保持在受控入口内，不进入公开信息流。',
      '资源与首页继续收口：新增 AI 音乐资源页，调整资源导航、筛选和壁纸入口；首页改为更轻的晨间城市氛围，并同步更新品牌、产品链接与社交预览。',
      '内容页统一阅读布局，精简站点说明与个人资料层级；最新灵感流加入 Codex 5.6 三档模型能力动效案例。',
    ],
  },
  {
    version: 'v2026.30',
    ...
    planned: [
      ...
      '站长想说：账号系统的目标是「珍惜每次输入、减少阻碍」。邮箱首次登录先创建未验证但可用的账号，立即保留阅读记录并发放 100 燃币；激活邮箱只是换设备登录、找回密码和长期权益的增强，不再作为继续使用的硬门槛。',
  ...
  },
  {
    ...
    range: '2026-06-19 起',
    commits: 4,
    title: '内容战略转向第一人称：奶爸身份线与后台打磨',
    summary:
      '这一轮不是堆功能，而是一次方向校正：把首页从「前端 AI 工程师」的通用人设转为第一人称复合身份（前端·AI·奶爸），新开 Dad Stack 把父亲这条差异化主线单独立起来，同时修掉后台调研风格页的布局问题。核心判断是——能被 AI 一键生成的内容已是 commodity，真正稀缺的是只有这个具体的人才写得出的第一人称经验。',
    planned: [
      '后台样式系统性巡检：继 research-style 两栏修复后，逐页核对 users / portfolio / ops 等控制台的布局与交互一致性。',
      '充实 Dad Stack 线：把循证育儿做成系列，逐步沉淀只有亲历者能写的第一人称育儿与家庭经验。',
      '梳理 6-8 篇第一人称选题（政企真实场景、前端转 AI 的真实路径、奶爸身份对成功定义的改变等），让 /articles 主推位从 AI 协助调研让位给第一人称叙事。',
      '补齐自有用户池：让 newsletter 从页面文案变成可用的邮件捕获实体。',
    ],
    done: [
      '首页 hero 转向第一人称复合身份：主标题改为「前端 · AI 工程 · 一个奶爸」，副标题「写代码，养家，做长期主义的产品」，简介加入「一岁半孩子的爸爸」，英文 hero 同步。',
      '新增 /dad-stack 育儿线聚合页：第一人称框定 AI × 工程 × 父亲，聚合两篇循证育儿调研与家庭工具，并接入「作品」频道导航。',
      '新增两篇循证育儿调研：《父亲如何科学育儿一岁半幼儿》与《孩子哭闹、妻子也烦躁时，丈夫怎么办》，按调研风格 v3 范式写作，附权威机构与同行评议来源。',
      '修复后台调研风格模版页：补回两栏布局（时间线左栏 sticky）、对照模式改为并排、版本排序由字符串比较改为数值比较。',
      '据实克制：私有奶爸待办不在公开页摆出，Dad Stack 文案明示这条线尚早，不假装成熟产品线。',
    ],
  },
  {
    version: 'v2026.26',
    week: '2026-W25',
    range: '2026-06-14 至 2026-06-18',
    commits: 79,
    title: '工程项目地图、实时专题与后台运营能力扩展',
    summary:
      '这一轮继续把站点从内容展示推进为可持续运营的产品系统：重构工程作品入口，上线世界杯和行情分析专题，并补齐模型调度、自动化与内容数据后台。',
    planned: [
      '完成 admin.2aran.com 自定义域绑定，让后台管理入口与公开站点彻底分离。',
      '为 Admin 子域接入 Cloudflare Access，补齐独立后台的访问控制。',
      '继续推进内容层统一：减少页面内硬编码数据，为后续 CMS 或独立内容仓库预留迁移路径。',
    ],
    done: [
      '重构 /works 为分层项目地图，并合并原 /ai-projects：用不同视觉权重呈现长期产品、AI 系统、工程作品和生活工具。',
      '新增 Agent 世界杯专题，接入 openfootball 数据源、D1 存储与 GitHub Actions 定时采集，并完成队名、轮次、场馆汉化和国旗展示。',
      '新增多标的行情分析页，支持 LOBSTERUSDT 分钟级与日线快照、MA 周期和持仓量观察，并修正指标卡和告警区布局。',
      '后台新增 DeepSeek 模型调度控制台：支持流式规划、步骤拆解和 Agent、Token、成本、耗时预估，同时抽出共享调用层。',
      '增强 Admin Ops 自动化控制台，建立自动化注册表，登记世界杯采集任务，并加入本地 Admin 预览模式。',
      '新增内容数据周报看板，汇总内容表现和榜单数据，并修复长 key 撑宽溢出。',
      '新增舆情分析工作台、R2 壁纸图库和任伟写作 Skill；补充 AGI-like Agent、自残式技术债、渠道先于产品与 DeepSeek V4 Pro 等调研。',
      '首页加入吸顶热点 ticker，重做关于页为全屏多段式布局；统一导航主题色、页面容器宽度，并优化移动端首屏间距。',
      '上线站点外壳与首页中英双语，按访问地区提供默认语言，并修复英文推荐卡和头像占位文案。',
      '同步加密记忆快照与索引，继续完善项目上下文的可追溯归档。',
    ],
  },
  {
    version: 'v2026.25',
    ...
    planned: [
      ...
      '压缩广州城市横幅图并接入首页；推荐卡改为可随机切换博主联盟、前端周看和AI分发大师。',
  ...
  },
  {
    ...
    items: [
      '首页主入口聚焦三个长期产品：博主联盟、前端周看、AI分发大师，并移除关注矩阵和最新动态等干扰模块。',
      ...
      '主导航文案改为“读文章 / 看项目 / 来合作 / 关于我”，并把AI分发大师提升到和博主联盟、前端周看同级的产品入口。',
    ],
  },
  {
    version: 'v2026.23',
