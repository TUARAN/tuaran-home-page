...
export default function ClassicalMasterpiecesClient() {
  const allWorks = useMemo(() => flattenWorks(), [])
  const [activeCategory, setActiveCategory] = useState('all')
  const [selectedWork, setSelectedWork] = useState(allWorks[0])

  const visibleWorks = useMemo(() => {
    return activeCategory === 'all' ? allWorks : allWorks.filter((work) => work.categoryId === activeCategory)
  }, [activeCategory, allWorks])

  const selectedCategory = CATEGORIES.find((category) => category.id === selectedWork.categoryId)
        ...
        <div className="relative mx-auto grid max-w-6xl gap-8 px-4 py-10 md:grid-cols-[1.1fr_0.9fr] md:py-14">
          ...
          <div className="self-end rounded-lg border border-[#e2d5c1] bg-white/78 p-4 shadow-[0_24px_70px_rgba(83,64,33,0.14)] backdrop-blur dark:border-[#293545] dark:bg-[#111923]/80 dark:shadow-none">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="mb-1 text-xs text-[#827664] dark:text-gray-400">当前高亮</p>
            ...
            <p className="mb-4 text-sm leading-7 text-[#51483b] dark:text-gray-300">{selectedWork.detail}</p>
            <div className="flex flex-wrap gap-1.5">
              {selectedWork.tags.map((tag) => (
                <span key={tag} className="rounded-full bg-[#f3eadc] px-2 py-1 text-[11px] text-[#736653] dark:bg-[#17212d] dark:text-gray-300">
                  {tag}
                </span>
              ))}
            </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-[1fr_320px]">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {visibleWorks.map((work) => (
            <WorkCard
              key={work.key}
              work={work}
              selected={selectedWork.key === work.key}
        ...
        <aside className="lg:sticky lg:top-[150px] lg:self-start">
          <div className="rounded-lg border border-[#e4d9c8] bg-white p-4 dark:border-[#283443] dark:bg-[#101820]">
            ...
            <h2 className="mb-1 border-b-0 pb-0 font-serif text-2xl font-semibold text-[#231f18] dark:text-gray-100">
              {selectedWork.title}
            </h2>
