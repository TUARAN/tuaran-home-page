...
export default function UnlockForm({ encryptedCount, password, onPasswordChange, onSubmit, busy, error }) {
  return (
    <section className="mt-8 grid gap-5 lg:grid-cols-[1fr_1.2fr]">
      <div className="rounded-lg border border-[#dee0db] bg-white/82 p-6 dark:border-gray-800 dark:bg-[#121821]/82">
        ...
        <form onSubmit={onSubmit} className="mt-4 flex flex-col gap-3">
          <input
            ...
            autoComplete="off"
            placeholder="资料库口令"
            className="rounded-lg border border-[#c5c7bb] bg-white px-3 py-2.5 text-sm text-[#15140f] outline-none focus:border-[#818472] dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100"
          />
          <button
            type="submit"
            disabled={busy || !password.trim()}
            className="rounded-lg bg-[#2f3027] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#15140f] disabled:cursor-not-allowed disabled:opacity-55 dark:bg-gray-200 dark:text-[#111]"
          >
            {busy ? '解锁中...' : '解锁'}
          </button>
        </form>
      ...
      <aside className="rounded-lg border border-dashed border-[#c5c7bb] bg-white/55 p-6 text-sm leading-7 text-[#51514a] dark:border-gray-700 dark:bg-[#121821]/55 dark:text-gray-300">
        <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#767869] dark:text-[#8e9ab0]">
          这里写了什么
        </p>
        <h3 className="mt-2 font-serif text-base font-semibold text-[#15140f] dark:text-gray-100">
          长期罗盘 · v3 摘要
        </h3>
        <p className="mt-3">
        ...
        <p className="mt-3 text-[12px] text-[#717367] dark:text-gray-400">
