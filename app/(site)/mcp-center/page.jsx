...
export const metadata = {
  title: 'MCP 中心',
  description: '面向 AI 智能体的 MCP 服务中心：连接公开文章与天气查询测试服务。',
...
const SECURITY_ITEMS = [
  ['只读最小权限', '仅提供公开文章元数据与公开天气查询；不暴露正文草稿、后台数据、用户信息和任何写操作。'],
  ['协议与输入校验', '校验 JSON-RPC、MCP 版本、Content-Type、请求体大小、工具名和参数范围。'],
  ['按服务区分授权', '文章 MCP 使用 OAuth 与 PKCE；天气测试 MCP 无需登录，只开放无副作用的公共查询。'],
...
export default function McpCenterPage() {
  return (
    <PageContainer className="py-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mb-0 max-w-3xl text-base leading-8 text-[#43433b] dark:text-gray-300">
              Skill 告诉智能体“怎么做”，MCP 让智能体“能连接什么”。这里既有需要授权的内容服务，也有无需登录、可快速验证客户端连接的天气测试服务。
            </p>
          </div>
      ...
      <section className="mt-8 rounded-lg border border-[#d2d3c8] bg-white dark:border-[#283443] dark:bg-[#101820]">
        <header className="border-b border-[#dedfd5] p-5 dark:border-[#263241]">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="mb-1 font-mono text-xs text-[#8b5a1f] dark:text-[#a1ab76]">tuaran-weather-test</p>
              <h2 className="mb-2 border-b-0 pb-0 font-serif text-2xl font-semibold text-[#1c1d18] dark:text-gray-100">天气查询测试 MCP</h2>
              <div className="flex flex-wrap gap-1.5">
                <Pill>测试服务</Pill><Pill>Streamable HTTP</Pill><Pill>无需登录</Pill><Pill>公开只读</Pill>
              </div>
            </div>
            <McpConfigActions config={WEATHER_JSON_CONFIG} />
          </div>
          <p className="mb-0 mt-4 max-w-3xl text-sm leading-7 text-[#4c4c44] dark:text-gray-300">
            用于验证客户端能否正确连接和调用 MCP。提供 <code>get_current_weather</code> 与 <code>get_weather_forecast</code>，支持按中英文城市名查询；数据来自 Open-Meteo，无需账号或 API Key。
          </p>
        </header>
        ...
        <div className="grid gap-6 p-5 lg:grid-cols-2">
          <section>
            <p className="mb-2 text-xs uppercase tracking-[0.12em] text-[#6e7064] dark:text-gray-400">通用客户端配置</p>
            <pre className="overflow-x-auto rounded-md border border-[#dedfd5] bg-[#f8f8f5] p-4 font-mono text-xs leading-6 text-[#33352d] dark:border-[#263241] dark:bg-[#0d151e] dark:text-gray-300"><code>{WEATHER_JSON_CONFIG}</code></pre>
            <p className="mt-3 text-xs leading-6 text-[#56564d] dark:text-gray-300">
              这是公开测试端点，添加后应能直接完成工具发现和调用，不会跳转 OAuth 授权页。
            </p>
          </section>
          <section>
            <p className="mb-2 text-xs uppercase tracking-[0.12em] text-[#6e7064] dark:text-gray-400">Codex config.toml</p>
            <pre className="overflow-x-auto rounded-md border border-[#dedfd5] bg-[#f8f8f5] p-4 font-mono text-xs leading-6 text-[#33352d] dark:border-[#263241] dark:bg-[#0d151e] dark:text-gray-300"><code>{WEATHER_CODEX_CONFIG}</code></pre>
            <div className="mt-4 rounded-md border border-[#dedfd5] bg-[#f8f8f5] p-4 dark:border-[#263241] dark:bg-[#121a24]">
              <p className="mb-1 text-xs text-[#6e7064] dark:text-gray-400">可以这样问</p>
              <p className="mb-0 text-sm leading-7 text-[#34362e] dark:text-gray-200">“调用 tuaran-weather-test，查询广州现在的天气和未来三天预报。”</p>
            </div>
          </section>
        </div>
      </section>
      ...
      <section className="mt-8 rounded-lg border border-[#d2d3c8] bg-white p-5 dark:border-[#283443] dark:bg-[#101820]">
        ...
        <p className="mb-0 mt-4 text-sm leading-7 text-[#4c4c44] dark:text-gray-300">
          文章服务中的站点登录会话、OAuth 授权服务与 MCP Resource Server 分开运行：Cookie 只用于确认用户身份，文章 MCP 只接受面向自身 audience、包含 <code>articles:read</code> scope 的短期 Access Token。天气测试服务不接触站内数据，以独立端点和更严格的公开限流运行。若未来接入付费内容或写操作，将继续拆分独立 scope 和服务。
        </p>
      </section>

      <section className="mt-8 grid gap-4 md:grid-cols-3">
        {[
          ['1', '添加服务', '复制对应配置，让客户端连接端点并发现可用工具。'],
          ['2', '按需授权', '天气测试可直接调用；文章服务会打开登录与 OAuth 同意页。'],
          ['3', '开始查询', '直接向智能体提问，服务只返回各自公开、只读的数据。'],
