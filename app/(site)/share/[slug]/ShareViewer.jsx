...
export default function ShareViewer({ slug }) {
  ...
  if (phase === 'loading') {
    return (
      <main className="mx-auto w-full max-w-[720px] px-4 py-12">
        <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-[#9c8f79] dark:text-[#8e9ab0]">
          Encrypted Share
        </p>
        <p className="mt-3 text-sm text-[#75695a] dark:text-[#9aa6b6]">加载中…</p>
      </main>
    )
  }
  ...
  if (phase === 'error') {
    return (
      <main className="mx-auto w-full max-w-[720px] px-4 py-12">
        ...
        <h1 className="mt-3 font-serif text-xl font-semibold text-[#221f19] dark:text-gray-100">
          打不开
        </h1>
        <p className="mt-2 text-sm text-[#a34f47] dark:text-[#e6a29b]">{error}</p>
      </main>
    )
  }
  ...
  if (phase === 'fetched') {
    return (
      <main className="mx-auto w-full max-w-[560px] px-4 py-12">
        ...
        <p className="mt-3 text-[13px] leading-7 text-[#5d554a] dark:text-gray-300">
          这是一份端到端加密文档，<strong>服务器看不到内容</strong>，需要密码才能解锁。
          密码错误时只会在你的浏览器里失败，服务器既不知道你猜了什么，也不知道你猜对了没。
        </p>
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-[#3f392f] dark:text-gray-200">
              访问密码
            </span>
            <input
              type="password"
        ...
        <p className="mt-6 text-[11px] text-[#9c8f79] dark:text-[#8e9ab0]">
          提示：把密码加到链接末尾 <code className="rounded bg-[#f4ede0] px-1 py-px font-mono text-[10px] dark:bg-[#19212b]">#密码</code>{' '}
  ...
  return (
    <main className="mx-auto w-full max-w-[760px] px-4 py-8 md:py-12">
      ...
      <h1 className="mb-2 mt-3 font-serif text-2xl font-semibold text-[#221f19] dark:text-gray-100 md:text-3xl">
        {note?.title || '加密文档'}
      </h1>
      <p className="mb-6 text-[11.5px] text-[#9c8f79] dark:text-[#8e9ab0]">
      ...
      />
      <p className="mt-8 border-t border-[#e8dfd0] pt-4 text-[11px] text-[#9c8f79] dark:border-[#252e39] dark:text-[#8e9ab0]">
