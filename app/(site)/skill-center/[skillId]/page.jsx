...
export default async function SkillDetailPage({ params }) {
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-10">
      <header className="mb-6 border-b border-[#dee0db] pb-6 dark:border-[#202938]">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mb-3 border-b-0 pb-0 font-serif text-3xl font-semibold text-[#191915] dark:text-gray-100 md:text-4xl">
              {skill.title}
            </h1>
        ...
        <p className="mb-0 mt-4 text-base leading-8 text-[#43433b] dark:text-gray-300">{skill.desc}</p>
      </header>

      {/* 基础信息 */}
      <section className="mb-6 rounded-lg border border-[#d2d3c8] bg-white p-5 dark:border-[#283443] dark:bg-[#101820]">
        <p className="mb-3 text-xs uppercase tracking-[0.12em] text-[#6e7064] dark:text-gray-400">
          基础信息
        </p>
        <dl className="grid gap-4 text-sm leading-7 text-[#43433b] dark:text-gray-300 md:grid-cols-2">
          <div>
            <dt className="mb-1 font-semibold text-[#1c1d18] dark:text-gray-100">触发场景</dt>
            <dd>{skill.trigger}</dd>
          </div>
          <div>
            <dt className="mb-1 font-semibold text-[#1c1d18] dark:text-gray-100">输入要求</dt>
            <dd>{skill.inputs.join(' / ')}</dd>
          </div>
          <div>
            <dt className="mb-1 font-semibold text-[#1c1d18] dark:text-gray-100">产出格式</dt>
            <dd>{skill.outputs.join(' / ')}</dd>
          </div>
          <div>
            <dt className="mb-1 font-semibold text-[#1c1d18] dark:text-gray-100">验收标准</dt>
        ...
        <section className="mb-6 rounded-lg border border-[#d2d3c8] bg-white p-5 dark:border-[#283443] dark:bg-[#101820]">
          <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
            <div>
              ...
              <h2 className="mb-0 border-b-0 pb-0 font-serif text-xl font-semibold text-[#1c1d18] dark:text-gray-100">
                {content.label}
              </h2>
            </div>
          ...
          {content.type === 'versions' ? (
            <div className="grid gap-3">
              {content.items.map((version) => (
                <article
                  key={version.label}
                  className="rounded-md border border-[#dedfd5] bg-[#f8f8f5] p-4 dark:border-[#263241] dark:bg-[#121a24]"
                >
                  <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
                    <h3 className="mb-0 border-b-0 pb-0 text-base font-semibold text-[#1c1d18] dark:text-gray-100">
                      {version.label}
                    </h3>
                    <p className="mb-0 text-xs text-[#6e7064] dark:text-gray-400">{version.useCase}</p>
                  </div>
                  <p className="mb-0 text-sm leading-7 text-[#43433b] dark:text-gray-300">{version.text}</p>
                </article>
              ))}
            </div>
          ) : null}
              ...
              {content.items.map((rule, index) => (
                ...
                >
                  <div className="mb-2 flex items-baseline gap-2">
                    <span className="font-mono text-xs text-[#8b5a1f] dark:text-[#a1ab76]">
                  ...
                  <p className="mb-0 text-sm leading-7 text-[#43433b] dark:text-gray-300">{rule.body}</p>
                </li>
              ))}
            </ol>
          ) : null}
      ...
      {/* Codex 下载/复制 */}
      {hasCodex ? (
        <section className="rounded-lg border border-[#d2d3c8] bg-white p-5 dark:border-[#283443] dark:bg-[#101820]">
      ...
      <div className="mt-8 text-center">
        <Link
          href="/skill-center"
          className="inline-flex items-center gap-1 text-sm text-[#555640] no-underline transition-colors hover:!no-underline hover:text-[#1c1d18] dark:text-gray-400 dark:hover:text-gray-100"
        >
          ← 返回 Skill 中心
        </Link>
      </div>
