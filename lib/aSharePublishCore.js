/**
 * A 股公司观察 · 后台发布纯逻辑（无外部依赖，可单测）
 *
 * 草稿发布 = 把 D1 草稿转成 research/companies/<date>-a-share-<code>.md
 * 并提交到仓库 main 留存，再发布 D1 快照（不触发 Pages 构建）。本文件只做内容组装与校验，
 * GitHub 提交与 D1 状态更新见 lib/aSharePublisher.js。
 */

export const AUTO_PUBLISH_DELAY_MS = 3 * 24 * 60 * 60 * 1000
export const AUTO_PUBLISH_BATCH_LIMIT = 20

/** 待复核计时从草稿最后一次进入 pending 的时间（updated_at）开始。 */
export function autoPublishAt(draft) {
  const pendingAt = Number(draft?.updated_at)
  return Number.isFinite(pendingAt) && pendingAt > 0 ? pendingAt + AUTO_PUBLISH_DELAY_MS : null
}

export function isAutoPublishDue(draft, now = Date.now()) {
  const dueAt = autoPublishAt(draft)
  return draft?.status === 'pending' && dueAt !== null && dueAt <= Number(now)
}

export function isReviewPublishable(draft) {
  return draft?.status === 'pending' || draft?.status === 'reviewed'
}

/** 后台展示用：刚进入待复核时为「3天后自动发布」，到期后改为「已到期待自动发布」。 */
export function autoPublishLabel(autoPublishAtValue, now = Date.now()) {
  const dueAt = Number(autoPublishAtValue)
  if (!Number.isFinite(dueAt) || dueAt <= 0) return null
  if (dueAt <= Number(now)) return { due: true, daysLeft: 0, label: '已到期待自动发布' }
  const daysLeft = Math.max(1, Math.round((dueAt - Number(now)) / (24 * 60 * 60 * 1000)))
  return { due: false, daysLeft, label: `${daysLeft}天后自动发布` }
}

/** 校验草稿来源节是否存在（与 aShareResearchCore.validateDraft 同一口径）。 */
export function hasSourceSection(content) {
  return /^## .*(?:信息来源|资料来源|来源与说明)/mu.test(String(content || ''))
}

/**
 * 清除 frontmatter 之前的杂文（模型在联网检索前输出的中间内容）。
 * 找到首个行首为 `---` 的 frontmatter 起点，之前的文本全部丢弃；本身干净则原样返回。
 */
export function stripPreamble(content) {
  const text = String(content || '')
  if (/^---\r?\n/.test(text)) return text
  const match = /\r?\n---\r?\n/u.exec(text)
  if (!match || match.index < 0) return text
  return text.slice(match.index + 1)
}

/**
 * 兼容早期模型漏写 frontmatter 结束标记的历史草稿。
 * 仅在文章从 frontmatter 开始、没有合法闭合标记，且能找到正文首个二级标题时补线；
 * 后续仍由 validatePublishContent 校验全部必填字段，无法确定结构的内容保持原样并拒绝发布。
 */
export function repairUnclosedFrontmatter(content) {
  const text = String(content || '')
  if (!/^---\r?\n/u.test(text)) return text
  if (/^---\r?\n[\s\S]*?\r?\n---(?:\r?\n|$)/u.test(text)) return text
  const heading = /^##\s+/mu.exec(text)
  if (!heading || heading.index <= 4) return text
  return `${text.slice(0, heading.index).replace(/\s*$/u, '')}\n---\n\n${text.slice(heading.index)}`
}

/**
 * 把生成草稿转成待发布文章：只清除模型前置杂文，frontmatter 与正文保持原样。
 * review_ready / ad_eligible 均保持 false：
 *  - review_ready=true 会进入 AdSense 复审白名单并触发严格审计，A 股观察标题
 *    固定含「调研」且篇幅较短，不符合该白名单口径；
 *  - 发布只是进入内容管线上线，广告与质量复审另行处理（与已发布文章一致）。
 */
export function draftToArticleContent(content) {
  const text = repairUnclosedFrontmatter(stripPreamble(content))
  if (!/^review_ready:\s*false\s*$/mu.test(text)) {
    throw new Error('草稿 frontmatter 必须保持 review_ready: false（发布不自动置 true）。')
  }
  return text
}

/** 发布文件名：沿用本地流水线惯例 <日期>-a-share-<代码>.md。 */
export function publishFileName(draft) {
  const date = String(draft?.draft_date || '').trim()
  const code = String(draft?.code || '').trim()
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error(`draft_date 非法：${date}`)
  if (!/^\d{6}$/.test(code)) throw new Error(`证券代码非法：${code}`)
  return `${date}-a-share-${code}.md`
}

/** 发布文件对应的公开文章 slug（与 lib/research/loader.js fileToSlug 一致）。 */
export function publishSlug(fileName) {
  const match = /^\d{4}-\d{2}-\d{2}-(.+?)\.md$/u.exec(String(fileName || ''))
  if (!match) throw new Error(`文件名无法推导 slug：${fileName}`)
  return match[1]
}

/** 发布前最小校验：frontmatter 关键字段 + 长度 + 来源节。 */
export function validatePublishContent(content, { code, name } = {}) {
  const text = String(content || '')
  if (text.length < 200) throw new Error('发布内容过短，疑似空响应。')
  if (!/^---\r?\n/u.test(text)) throw new Error('发布内容必须以 frontmatter 开头（存在未清洗的模型中间输出）。')
  const frontmatterMatch = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/u.exec(text)
  if (!frontmatterMatch) throw new Error('发布内容的 frontmatter 未闭合（缺少结束标记 ---）。')
  const frontmatter = frontmatterMatch[1]
  const body = text.slice(frontmatterMatch[0].length)
  if (!hasSourceSection(body)) throw new Error('发布内容缺少结尾来源章节。')
  if (/\{\{[A-Z0-9_]+\}\}/u.test(text)) throw new Error('发布内容仍包含未替换的模板占位符。')

  const required = [
    [/^title:\s*/mu, 'title'],
    [/^category:\s*companies\s*$/mu, 'category 必须为 companies'],
    [/^company_type:\s*a_share\s*$/mu, 'company_type 必须为 a_share'],
    [/^stock_code:\s*"?\d{6}"?\s*$/mu, 'stock_code 必须是 6 位代码'],
    [/^exchange:\s*/mu, 'exchange'],
    [/^board:\s*/mu, 'board'],
    [/^date:\s*"?\d{4}-\d{2}-\d{2}"?\s*$/mu, 'date 必须为 YYYY-MM-DD'],
    [/^tags:\s*\[/mu, 'tags'],
    [/^summary:\s*\S/mu, 'summary'],
    [/^review_ready:\s*false\s*$/mu, 'review_ready 必须保持 false（发布不自动置 true，广告审核另行处理）'],
    [/^ad_eligible:\s*false\s*$/mu, 'ad_eligible 必须保持 false'],
  ]
  for (const [pattern, label] of required) {
    if (!pattern.test(frontmatter)) throw new Error(`发布校验失败：frontmatter 缺少 ${label}。`)
  }
  if (code && !new RegExp(`stock_code:\\s*"?${code}"?`, 'u').test(frontmatter)) {
    throw new Error(`发布校验失败：stock_code 与草稿 ${code} 不一致。`)
  }
  if (name && !text.includes(String(name))) {
    throw new Error(`发布校验失败：正文未包含公司名「${name}」。`)
  }
  return true
}
