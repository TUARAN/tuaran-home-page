...
export async function autoPublishOldestDueDraft({ db, env, now = Date.now() }) {
  ...
  const draft = await db
    .prepare("SELECT * FROM a_share_drafts WHERE status = 'pending' AND updated_at <= ? ORDER BY updated_at ASC LIMIT 1")
    .bind(cutoff)
    .first()
  if (!draft) return { ok: true, skipped: true, reason: 'none-due' }
  return publishAShareDraft({ db, env, draft, mode: 'auto', now })
}
