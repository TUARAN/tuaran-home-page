import { prepareResearchSourcePublication, commitResearchSourcePublication } from './researchSourcePublication.js'
/** A 股草稿共享发布服务：供后台人工发布与定时自动发布共同调用。 */

import {
  AUTO_PUBLISH_BATCH_LIMIT,
...
export async function publishAShareDraft({ db, env, draft, mode = 'manual', now = Date.now() }) {
  ...
  try {
    ...
    const publication = await prepareResearchSourcePublication(db, { category: 'companies', filename: fileName, raw: article })
    ...
    const commit = await githubJson(`https://api.github.com/repos/${repo}/contents/${path}`, token, {
      ...
      body: JSON.stringify({
        message: `[CF-Pages-Skip] content: ${mode === 'auto' ? 'auto-' : ''}publish A-share observation ${draft.name} ${draft.code}`,
        content: base64EncodeUtf8(article),
        branch: 'main',
        ...(existingSha ? { sha: existingSha } : {}),
      }),
    })
    await commitResearchSourcePublication(db, publication)
...
/** 每次调度发布全部已到期待复核稿，坏稿自动退回，缺 token 时停止以免重复失败。 */
export async function autoPublishOldestDueDraft({ db, env, now = Date.now() }) {
  const cutoff = now - AUTO_PUBLISH_DELAY_MS
  const { results } = await db
    .prepare(`SELECT * FROM a_share_drafts WHERE status = 'pending' AND updated_at <= ? ORDER BY updated_at ASC LIMIT ${AUTO_PUBLISH_BATCH_LIMIT}`)
    .bind(cutoff)
    .all()
  ...
  for (const draft of drafts) {
    try {
      ...
      if (result?.ok && !result.skipped) published.push(result)
    } catch (error) {
      if (error instanceof ASharePublishError && error.code === 'INVALID_CONTENT') {
        await db
      ...
      if (error instanceof ASharePublishError && error.code === 'PUBLISH_TOKEN_MISSING') {
        fatalError = error
        break
      }
  ...
  if (published.length) {
    return {
      ...
      ...published[0],
      published,
      rejectedInvalidDrafts: rejected,
    }
  }
  if (rejected.length && !fatalError) {
    return { ok: true, skipped: true, reason: 'invalid-content-rejected', rejectedInvalidDrafts: rejected }
  }
  if (fatalError) throw fatalError
  return { ok: true, skipped: true, reason: 'none-due', rejectedInvalidDrafts: rejected }
}
