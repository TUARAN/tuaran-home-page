...
async function countPoolRows(db) {
  const row = await db.prepare('SELECT COUNT(*) AS count FROM a_share_pool').first()
...
export async function syncPoolToD1(db, { force = false, staleDays = DEFAULT_STALE_DAYS, now = Date.now() } = {}) {
  ...
  if (!force && snapshot?.snapshot_date) {
    ...
    if (now - Number(snapshot.generated_at) < staleMs) {
      return { status: 'fresh', snapshotDate: snapshot.snapshot_date, count: Number(snapshot.count) || (await countPoolRows(db)) }
    }
  }
  ...
  const statements = [db.prepare('DELETE FROM a_share_pool')]
  const CHUNK = 80
  for (let index = 0; index < active.length; index += CHUNK) {
    const batch = active.slice(index, index + CHUNK)
    statements.push(
      ...batch.map((company) =>
        db
          .prepare(
            'INSERT INTO a_share_pool (code, name, exchange, board, snapshot_date, generated_at) VALUES (?, ?, ?, ?, ?, ?)',
          )
          .bind(company.code, company.name, company.exchange, company.board, date, generatedAt),
      ),
    )
  }
  ...
  await db
    .prepare(
      `INSERT INTO a_share_pool_snapshot (id, snapshot_date, count, generated_at) VALUES (1, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET snapshot_date = excluded.snapshot_date,
         count = excluded.count, generated_at = excluded.generated_at`,
    )
    .bind(date, active.length, generatedAt)
    .run()
...
export async function selectNextCompany(db, { now = Date.now() } = {}) {
  const pending = await db
    ...
    .first()
  if (pending) {
    const company = await db.prepare('SELECT * FROM a_share_pool WHERE code = ?').bind(pending.code).first()
    if (!company) throw new Error(`待完成公司 ${pending.code} 已不在当前公司池，请人工处理状态。`)
    return { company, selection: pending, resumed: true }
  }

  const { results: allCompanies } = await db.prepare('SELECT * FROM a_share_pool ORDER BY code').all()
...
export async function runAShareDaily({ db, env, forceSync = false, staleDays = DEFAULT_STALE_DAYS }) {
  ...
  try {
    ...
    if (draft?.status === 'generating' && startedAt - Number(draft.updated_at) < GENERATION_LOCK_MS) {
      await recordRun(db, {
        action: 'draft',
        code: picked.company.code,
        companyName: picked.company.name,
