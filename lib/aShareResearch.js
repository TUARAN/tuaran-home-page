...
export async function runAShareDaily({ db, env, forceSync = false, staleDays = DEFAULT_STALE_DAYS }) {
  ...
  try {
    const autoPublish = await autoPublishOldestDueDraft({ db, env, now: startedAt })
    ...
    if (
      ...
      return { ok: true, phase: 'generation-locked', company: picked.company.code, draftId, autoPublish }
    }
