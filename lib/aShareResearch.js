 ...
 *
 * 自动生成稿保持 review_ready: false / ad_eligible: false；进入待复核满 3 天仍未处理时，
 * 定时任务按进入待复核的时间顺序自动发布全部到期草稿。站长可在到期前发布或退回阻止自动发布。
 */

import { RESEARCH_STYLE_TEMPLATES } from './researchStyleTemplates'
import { callDeepSeekResponses } from './deepseek'
import {
  A_SHARE_RESEARCH_TEMPLATE_VERSION,
  CNINFO_STOCK_LIST,
  DEFAULT_STALE_DAYS,
...
import { autoPublishOldestDueDraft } from './aSharePublisher'
...
async function bumpDraftAttempt(db, draftId) {
  await db
    .prepare("UPDATE a_share_drafts SET attempt_count = attempt_count + 1, status = 'generating', generation_error = '', updated_at = ? WHERE id = ?")
    .bind(Date.now(), draftId)
    .run()
}

async function markDraftFailed(db, draftId, error) {
  if (!draftId) return
  await db
    .prepare("UPDATE a_share_drafts SET status = 'failed', generation_error = ?, updated_at = ? WHERE id = ? AND status = 'generating'")
    .bind(String(error?.message || error).slice(0, 2000), Date.now(), draftId)
    .run()
}
...
async function createDraftRow(db, { company, date, now }) {
  ...
  await db
    ...
    .bind(
      ...
      company.code,
      company.name,
      `阿燃调研：每天一家A股上市公司 —— ${company.name}（${company.code}）公司观察`,
      date,
      String(A_SHARE_RESEARCH_TEMPLATE_VERSION),
      now,
      now,
    )
    .run()
...
export async function runAShareDaily({ db, env, forceSync = false, staleDays = DEFAULT_STALE_DAYS }) {
  ...
  let activeDraftId = ''
  try {
    // 发布配置或历史坏稿不能阻断当天的新选题；发布服务会单独记录失败日志。
    const autoPublish = await autoPublishOldestDueDraft({ db, env, now: startedAt }).catch((error) => ({
      ok: false,
    ...
    if (sync.status === 'synced') {
      ...
      return { ok: true, phase: 'pool-synced', autoPublish, ...sync }
    }
    ...
    const generationDecision = draftGenerationDecision(draft, startedAt)
    if (generationDecision.action === 'done') {
      await recordRun(db, {
        action: 'draft',
      ...
      return { ok: true, phase: 'already-done', company: picked.company.code, draftId, autoPublish }
    }
    if (generationDecision.action === 'stop') {
      throw new Error(`草稿 ${draftId} 当前状态 ${draft.status} 不允许继续生成。`)
    }
    if (generationDecision.action === 'exhausted') {
      throw new Error(`草稿 ${draftId} 已重试 ${generationDecision.attempts} 次仍未完成，转为人工处理。`)
    }
    if (generationDecision.action === 'locked') {
      await recordRun(db, {
        action: 'draft',
        code: picked.company.code,
        companyName: picked.company.name,
      ...
      return {
        ok: true,
        phase: 'generation-locked',
        company: picked.company.code,
        draftId,
        retryAfterMs: Math.max(1_000, generationDecision.retryAfterMs),
        autoPublish,
      }
    }
    ...
    const result = await callDeepSeekResponses({
      env,
      // Responses API 直接接受 [{role, content}] 输入项；system 项等同 instructions。
      input: messages,
      // 服务端联网检索：老 /chat/completions 会忽略 web_search，必须走 Responses API。
      tools: [{ type: 'web_search' }],
      toolChoice: 'auto',
      temperature: 0.35,
      maxTokens: 2816,
      timeoutMs: GENERATION_TIMEOUT_MS,
      // 起草是短文任务，默认走 flash；任务声明优先于密钥上的 default_model。
      taskDefaultModel: 'deepseek-v4-flash',
      // 关闭思考模式：Responses API 用 reasoning.effort = none；
      // 不关的话 DeepSeek V4 默认把 token 花在 reasoning_content，墙钟内写不完正文。
    ...
    // 模型偶尔会从正文一级标题直接继续，漏掉 frontmatter 的第二条 `---`。
    // 发布链路已有保守修复；生成链路也必须在校验和落库前走同一修复，
    // 否则可恢复的输出会被连续计为失败，直到耗尽重试次数。
    const article = repairUnclosedFrontmatter(stripPreamble(result.content))
    validateDraft(article)
    const updatedAt = Date.now()
    await db
      .prepare(
        `UPDATE a_share_drafts SET content = ?, status = 'pending', generation_error = '', deepseek_task_id = ?, updated_at = ?
         WHERE id = ?`,
      )
      .bind(article.slice(0, 200_000), result.taskId || '', updatedAt, draftId)
      .run()
    await db
      .prepare(
    ...
    return { ok: true, phase: 'drafted', company: picked.company.code, name: picked.company.name, draftId, taskId: result.taskId || '', autoPublish }
  } catch (error) {
    await markDraftFailed(db, activeDraftId, error).catch(() => {})
    await recordRun(db, {
      action,
      status: 'failed',
      error: String(error?.message || error).slice(0, 2000),
