...
export const A_SHARE_RESEARCH_TEMPLATE_VERSION = 5
...
export function draftGenerationDecision(draft, now = Date.now()) {
  ...
  // 旧生成链路没有调用 frontmatter 修复器，导致这类本可恢复的草稿耗尽 5 次。
  // 修复上线后只额外放行一次；若第 6 次仍失败，继续进入 exhausted，避免无限重试。
  const recoverableLegacyFormatError = /frontmatter 未闭合（缺少结束标记 ---）/u.test(
    String(draft.generation_error || ''),
  )
  if (attempts === MAX_DRAFT_ATTEMPTS && recoverableLegacyFormatError) return { action: 'attempt' }
  if (attempts >= MAX_DRAFT_ATTEMPTS) return { action: 'exhausted', attempts }
  ...
  return [
    {
      ...
      content:
        ...
        '每条关键事实必须能追溯到用户提供的资料或你实际检索到的来源。控制权、最新业绩等会改变判断的关键项检索不到，才写入文末「持续验证」短列表；不影响判断的缺口不必凑条数。不得编造财务数字、并购重组、管理层变动、' +
        '业绩预期等信息，也不得编造来源 URL。全程最多执行 2 次联网检索，优先覆盖主营业务与最新业绩；' +
        '检索结果直接服务写作，不要在正文里复述检索过程。' +
        '输出完整 Markdown 文章，包含 frontmatter 与九个小节；必须写完「九、信息来源与持续验证」再结束；' +
        '该节先写来源，持续验证只保留会改变判断的缺项；篇幅按重要性分配，不要为凑齐骨架注水，也不得漏掉结尾来源节；不要输出代码围栏，不要输出任何额外说明。',
    },
    {
      role: 'user',
      content: [
        ...
        quoteTextFor(quote),
        '',
        '写作模板（九个小节必须全部出现，章节标题保持以下编号与名称）：',
        sections.map((section) => `## ${section}`).join('\n'),
        '',
        'frontmatter 要求：',
        '- 第一行必须是 `---`；全部 frontmatter 字段写完后必须另起一行再写 `---`，然后空一行开始 `## 一、先给结论`。两条 `---` 都不可省略。',
        [
        ...
        [
          ...
          '凡在正文使用检索结果，必须在「九、信息来源与持续验证」列出对应来源（网站名 + URL），URL 只能来自本次检索结果，不得编造。',
          '「二、公司身份与上市信息」只填已经能确认的项目；不影响认出这家公司的空项可以留空，不必写成待核实清单。',
          '控制权为必查项：写出直接控股股东及持股比例，逐层穿透至最终控制人，并标明民营、央企或地方国资；地方国资须进一步写明省级、市级或区县级及具体履职机构。',
          '核对最近一期定期报告之后的权益变动、无偿划转、收购报告书和控制权变更公告；说明中间持股层级变化是否改变直接控股股东或实际控制人。控制权说不清楚时在文末记一条，其它不影响判断的缺口不必列。',
          '「四、财务质量」没有可核实的年报数据时，表格留空；只有这会改变经营判断时，才在文末持续验证中说明。',
          '「六、估值与市场预期」使用提供的行情数据并写明口径日期。',
          '「八、外部研判」只写基于上面事实的观察，不替公司或投资者推断动机。点到为止。',
          '「九、信息来源与持续验证」先写来源：巨潮资讯公司列表、腾讯行情接口、本次检索实际使用的来源（名称 + URL），资料截至今天；关键财务数据以公司正式披露为准。持续验证只保留会改变判断的缺项。',
...
export function validateDraft(content) {
  ...
  if (!/^---\r?\n/u.test(content)) {
    throw new Error('草稿必须以 frontmatter 开头。')
  }
  const frontmatterMatch = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/u.exec(content)
  if (!frontmatterMatch) {
    throw new Error('草稿 frontmatter 未闭合（缺少结束标记 ---）。')
  }
  const frontmatter = frontmatterMatch[1]
  if (/\{\{[A-Z0-9_]+\}\}/u.test(content)) throw new Error('草稿仍包含未替换的模板占位符。')
  if (!/^review_ready:\s*false\s*$/mu.test(frontmatter)) throw new Error('自动生成稿必须保持 review_ready: false。')
  if (!/^ad_eligible:\s*false\s*$/mu.test(frontmatter)) throw new Error('自动生成稿必须保持 ad_eligible: false。')
