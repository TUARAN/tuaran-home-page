...
import { taxonomyForArticle } from './contentTaxonomy'
...
export async function listPublishedArticlePosts({ metadataOnly = false } = {}) {
  const db = dbOrNull()
  if (!db) return []
  try {
    const result = await db
      .prepare(
        `SELECT ${metadataOnly
          ? 'id, slug, title, summary, cover_url, tags_json, status, created_at, updated_at, published_at, substr(content_text, 1, 160) AS content_text'
          : '*'} FROM article_posts
