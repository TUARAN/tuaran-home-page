...
function riskFlags(url, title) {
  ...
  try {
    ...
    if (parsed.protocol === 'http:') flags.push('insecure-http')
...
export function parseChromeBookmarks(html) {
  ...
  for (const entry of entries) {
    const first = firstByUrl.get(entry.url)
    if (first) entry.duplicateOf = first
    else firstByUrl.set(entry.url, entry.id)
  }
...
export function summarizeBookmarks(entries) {
  ...
  for (const entry of list) {
    ...
    if (entry.duplicateOf) duplicateEntries += 1
