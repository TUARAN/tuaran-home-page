...
function homeVisibilityThrough(event) {
  if (event?.status === 'announced') {
    return beijingDateFromTimestamp(event?.schedule?.through)
      || beijingDateFromTimestamp(event?.schedule?.from)
      || calendarDateForEvent(event)
  }
  return beijingDateFromTimestamp(event?.confirmedAt)
    || event?.occurredOn
    || calendarDateForEvent(event)
}

/** 首页只推荐仍有效的预告，或当天刚确认的事件。历史记录留在详情页。 */
export function pickHomeFeaturedEvent(events, today = todayBeijing()) {
  const eligible = (events || []).filter((event) => {
    const through = homeVisibilityThrough(event)
    return through && through >= today
  })
  return pickFeaturedEvent(eligible, today)
}
...
export function homeCardModel(snapshot, today = todayBeijing()) {
  ...
  const featured = pickHomeFeaturedEvent(events, today)
  const date = featured ? calendarDateForEvent(featured) : null
  const post = latestPost(featured)
  return {
    featured,
    date,
    kind: featured ? eventKind(featured) : 'preview',
    kicker: featured ? featuredKicker(featured) : '查看历史',
...
export function homeStripHeadline(model, today = todayBeijing()) {
  if (!model?.featured) return '暂无新预告'
