...
const COMMUNITY_TOPIC_NAV_COPY = {
  'x-mutual-aid-circle': {
    label: 'X 互助圈',
    labelEn: 'X Circle',
    desc: '互动与账号增长',
    descEn: 'Engagement & account growth',
  },
  'xiaohongshu-creator-circle': {
    label: '小红书创作圈',
    labelEn: 'Xiaohongshu Circle',
    desc: '选题、标题与封面互评',
    descEn: 'Topics, titles & cover reviews',
  },
  'juejin-creator-circle': {
    label: '掘金创作圈',
    labelEn: 'Juejin Circle',
    desc: '技术文章互审共创',
    descEn: 'Technical writing review',
  },
}

export const DISCUSSION_COMMUNITY_TOPICS = COMMUNITY_TOPICS
  .filter((topic) => COMMUNITY_TOPIC_NAV_COPY[topic.id])

export const COMMUNITY_TOPIC_NAV_ITEMS = DISCUSSION_COMMUNITY_TOPICS.map((topic) => {
  const { tag: _tag, ...item } = topic
  return { ...item, ...COMMUNITY_TOPIC_NAV_COPY[topic.id] }
})
