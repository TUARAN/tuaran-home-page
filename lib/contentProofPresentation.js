const DEFAULT_EXPECTED_CHAIN_ID = 84532
...
export function describeProofStatus({ proof, verification, batch, expectedChainId = DEFAULT_EXPECTED_CHAIN_ID } = {}) {
  ...
  if (batch.anchor?.chainId !== expectedChainId) {
    return { level: 'error', label: '链网络不匹配', detail: `预期 chainId ${expectedChainId}，实际为 ${batch.anchor?.chainId ?? '未提供'}。` }
  }
