...
const SUBJECT_ALIASES = {
  life_practice: 'life_family',
}

const WEB3_TEXT_RE = /web3|crypto|blockchain|链上|加密资产|以太坊|比特币|智能合约|公链|erc-?20|solana|evm|svm|钱包|dapp|代币|稳定币|一枚币|哪条链|chain id|layer\s*[12]|spl token|pump\.fun|coinmarketcap/i
const WEB3_SLUG_RE = /(^|-)(crypto|web3|dapp|onchain|solana|ethereum|bitcoin|tron|ripple|tether|binancecoin)(-|$)|which-chain|public-chain|robinhood-chain|cryptocurrency|coinmarketcap|pump-fun|token-onchain|this-token-on/i

function aliasSubjects(values) {
  return (Array.isArray(values) ? values : [values]).map((value) => SUBJECT_ALIASES[value] || value)
}

function uniqueKnown(values, knownKeys, fallback) {
  const known = new Set(knownKeys)
  const result = [...new Set(aliasSubjects(values).filter((value) => known.has(value)))]
  if (!result.length && fallback) return [fallback]
  return result.slice(0, 1)
}

function looksLikeWeb3(entry = {}) {
  if (entry.category === 'companies' || entry.category === 'people') return false
  if (inferSeries(entry) === 'crypto_research') return true
  const tags = Array.isArray(entry.tags) ? entry.tags.join(' ') : String(entry.tags || '')
  const text = `${entry.slug || ''} ${entry.title || ''} ${entry.href || ''} ${tags} ${entry.cryptoType || ''} ${entry.kind || ''}`
  return WEB3_TEXT_RE.test(text) || WEB3_SLUG_RE.test(String(entry.slug || entry.href || ''))
}
...
/** 前台卡片只展示类型 + 主题，不把 research 目录名（主题/公司观察/人物）当标签。 */
export function publicTaxonomyLabels(record = {}) {
  const group = getContentGroup(record.contentKind)
  const subject = getDisplaySubject(record.subjects)
  return {
    group,
    groupLabel: CONTENT_GROUP_META[group]?.label || '内容',
    subject,
    subjectLabel: SUBJECT_META[subject]?.label || '',
  }
}
...
export function taxonomyForInteractive(work = {}) {
  ...
  const subjects = []
  if (/workbuddy/.test(text)) subjects.push('workbuddy')
  if (looksLikeWeb3(work)) subjects.push('web3')
  if (/ai|agent|模型|skill|智能/.test(text)) subjects.push('ai_dev')
  if (/工程|web|cloudflare|网络|架构|开发/.test(text) && !looksLikeWeb3(work)) subjects.push('web_cloud')
