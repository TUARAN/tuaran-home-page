...
export function taxonomyForResearch(entry = {}) {
  ...
  const isCryptoResearch = inferSeries(entry) === 'crypto_research'
  const subjects = (
    explicitSubjects.length
      ? explicitSubjects
      : uniqueKnown(
          [
            isCryptoResearch ? 'web3' : '',
...
export function taxonomyForInteractive(work = {}) {
  ...
  if (/web3|crypto|blockchain|链上|加密资产|以太坊|比特币|智能合约|公链/.test(text)) subjects.push('web3')
