...
export function buildContractRenewalPrintHtml(input) {
  ...
  const pages = briefing.pages.map((page) => `
    <section class="page">
      <h2>${page.id}　${escapeBriefingHtml(page.title)}　<span>${page.seconds}秒</span></h2>
      ${page.job ? `<p class="meta">${escapeBriefingHtml(page.job)}</p>` : ''}
      ${printLines(page.lines)}
      ${page.cutLines?.length ? `<p class="cut-label">压缩</p>${printLines(page.cutLines)}` : ''}
    </section>
  `).join('')
...
<head>
  ...
  <style>
    ...
    h1, h2 { font-family: "Heiti SC", "STHeiti", "SimHei", sans-serif; }
    header { display: flex; align-items: baseline; gap: 10pt; margin: 0 0 6pt; }
    h1 { font-size: 13pt; margin: 0; }
    .deck { margin: 0; font-size: 9.5pt; color: #555; }
    h2 {
      font-size: 10.5pt;
      margin: 7pt 0 2pt;
    ...
    h2 span, .meta, .cut-label { font-weight: 400; color: #666; }
    .meta, .cut-label { font-size: 9pt; margin: 0 0 2pt; }
    .cut-label { margin: 3pt 0 1pt; }
    p { margin: 0 0 3pt; }
    .page { margin: 0; break-inside: avoid; page-break-inside: avoid; }
