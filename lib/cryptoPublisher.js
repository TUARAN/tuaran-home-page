...
export async function publishCryptoDraft({ db, env, draft, mode = 'manual', now = Date.now() }) {
  ...
  try {
    ...
    const publication = await prepareResearchSourcePublication(db, { category: 'topics', filename: fileName, raw: article })
    ...
    const commit = await githubJson(`https://api.github.com/repos/${repo}/contents/${path}`, token, {
      ...
      body: JSON.stringify({
        message: `[CF-Pages-Skip] content: ${mode === 'auto' ? 'auto-' : ''}publish crypto observation ${draft.name} ${draft.symbol}`,
...
export async function autoPublishOldestDueCryptoDraft({ db, env, now = Date.now() }) {
  ...
  for (const draft of results) {
    try {
      const result = await publishCryptoDraft({ db, env, draft, mode: 'auto', now })
  ...
  if (!published.length) return { ok: true, skipped: true, reason: 'invalid-content-rejected' }
  return { ok: true, ...published[0], published }
}
