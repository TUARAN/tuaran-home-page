...
async function fetchMarketCoins(env) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(`CoinGecko markets 返回 HTTP ${response.status}`)
      error.name = 'CoinGeckoMarketError'
      error.status = response.status
      error.retryAfterMs = coingeckoRetryAfterMs(response.headers.get('retry-after'))
      throw error
    }
...
export async function syncCryptoPool(db, env, { force = false, now = Date.now() } = {}) {
  ...
  let coins
  try {
    coins = await fetchMarketCoins(env)
  } catch (error) {
    const fallbackCoins = parseCoins(current)
    if (error?.status === 429 && canUseCryptoPoolFallback({ generatedAt: current?.generated_at, coins: fallbackCoins, now })) {
      return {
...
export async function runCryptoDaily({ db, env, forceSync = false }) {
  ...
  try {
    ...
    const poolSync = sync.status === 'stale-fallback'
      ? { status: sync.status, snapshotDate: sync.snapshotDate, retryAfterMs: sync.retryAfterMs }
      : null
    if (poolSync) {
      await recordRun(db, {
        action,
        status: 'skipped',
        error: `CoinGecko 限流，沿用 ${sync.snapshotDate} 的有效快照`,
        startedAt,
      })
    }
    ...
    if (decision.action === 'done') return { ok: true, phase: 'already-done', coin: coin.id, draftId, autoPublish, poolSync }
    if (decision.action === 'stop') throw new Error(`草稿 ${draftId} 当前状态 ${draft.status} 不允许继续生成。`)
    if (decision.action === 'exhausted') {
      return { ok: true, phase: 'manual-review', coin: coin.id, draftId, attempts: decision.attempts, autoPublish, poolSync }
    }
    if (decision.action === 'locked') return { ok: true, phase: 'generation-locked', coin: coin.id, draftId, retryAfterMs: Math.max(1000, decision.retryAfterMs), autoPublish, poolSync }
    ...
    return { ok: true, phase: 'drafted', coin: coin.id, symbol: coin.symbol, name: coin.name, marketCapRank: coin.marketCapRank, draftId, taskId: result.taskId || '', autoPublish, poolSync }
  } catch (error) {
