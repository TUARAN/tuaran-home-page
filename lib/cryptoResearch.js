...
export async function runCryptoDaily({ db, env, forceSync = false }) {
  ...
  try {
    ...
    if (decision.action === 'exhausted') throw new Error(`草稿 ${draftId} 已重试 ${decision.attempts} 次，转为人工处理。`)
    ...
    if (draftId) await db.prepare("UPDATE crypto_drafts SET status = 'failed', generation_error = ?, updated_at = ? WHERE id = ? AND status = 'generating'").bind(String(error?.message || error).slice(0, 2000), Date.now(), draftId).run().catch(() => {})
    await recordRun(db, { action, coin, draftId, status: 'failed', error: error?.message || error, startedAt }).catch(() => {})
    throw error
  }
}
