...
export const CRYPTO_POOL_FALLBACK_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000
export const CRYPTO_RESEARCH_TEMPLATE_VERSION = 3
...
export function coingeckoRetryAfterMs(value, now = Date.now()) {
  const text = String(value || '').trim()
  if (!text) return 60_000
  const seconds = Number(text)
  if (Number.isFinite(seconds) && seconds >= 0) return Math.max(1000, Math.ceil(seconds * 1000))
  const retryAt = Date.parse(text)
  return Number.isFinite(retryAt) ? Math.max(1000, retryAt - Number(now)) : 60_000
}

export function canUseCryptoPoolFallback({ generatedAt, coins, now = Date.now() }) {
  const ageMs = Number(now) - Number(generatedAt || 0)
  if (ageMs < 0 || ageMs > CRYPTO_POOL_FALLBACK_MAX_AGE_MS) return false
  try {
...
const SOURCE_SECTION_TITLES = ['十、信息来源与持续验证', '十、信息来源与未能验证']

function hasRequiredSection(text, section) {
  const titles = section === '十、信息来源与持续验证' ? SOURCE_SECTION_TITLES : [section]
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        '',
        '写作约束：把已经能确认的机制、用途和风险写清楚，篇幅按重要性分配。每个关键事实附可访问的直接来源；区分协议、网络与代币；区分事实、项目方主张和外部判断。第十节先列 CoinGecko markets 接口和实际检索来源。会改变判断的缺失或冲突才写入节末「持续验证」；不影响判断的缺口不必列。禁用“不是 X，而是 Y”句式，删除无导航作用的“本文/本篇/本调研”。',
        '',
        styleText,
      ].join('\n'),
    },
...
export function validateCryptoDraft(content, coin = null) {
  ...
  for (const section of SECTIONS) if (!hasRequiredSection(text, section)) throw new Error(`草稿缺少章节：${section}。`)
