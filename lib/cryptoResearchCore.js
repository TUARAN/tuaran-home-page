...
export function draftGenerationDecision(draft, now = Date.now()) {
  ...
  if (attempts >= MAX_DRAFT_ATTEMPTS && !recoverableLegacyTimeout) return { action: 'exhausted', attempts }
...
export function validateCryptoDraft(content, coin = null) {
  ...
  const required = [
    [/^category:\s*topics\s*$/mu, 'category'],
    [/^crypto_type:\s*asset\s*$/mu, 'crypto_type'],
