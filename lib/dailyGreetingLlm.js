...
export function normalizeGreetingModelSelections(value, {
  ...
  const normalized = [...new Set((Array.isArray(items) ? items : [])
    ...
    .filter((item) => item === 'deepseek' || /^ollama:[^:]+$/u.test(item)))]
    .slice(0, 2)
...
export function parseGreetingModelSelection(value) {
  const id = String(value || '').trim()
  if (id === 'deepseek') return { id, provider: 'deepseek', providerId: '' }
  if (id.startsWith('ollama:') && id.length > 7) return { id, provider: 'ollama', providerId: id.slice(7) }
  return null
}
