 ...
 *   解析优先级：精确绑定 > source 绑定 > 全局兜底 > 环境变量默认密钥。
 */
...
export { ENV_DEFAULT_KEY_ID, ENV_DEFAULT_KEY_NAME, decryptApiKey, encryptApiKey, maskApiKey, parseBindings } from './deepseekKeysCore.js'
...
export async function resolveDeepSeekKey({ env, source = '', taskType = '' } = {}) {
  ...
  try {
    ...
    const best = pickBestKeyRow(results, String(source || ''), String(taskType || ''))
