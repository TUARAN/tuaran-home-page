 ...
 *   同一 D1 中的数据库密钥可被这些 source 复用；不同 Cloudflare 项目的环境变量不会自动同步。
 */
 ...
 */
export async function resolveDeepSeekKey({ env, source = '', taskType = '', trackUsage = true } = {}) {
...
/** 只检查当前运行环境能否解析出 Key，不回传明文，也不计入使用次数。 */
export async function hasUsableDeepSeekKey({ env, source = '', taskType = '' } = {}) {
  const resolved = await resolveDeepSeekKey({ env, source, taskType, trackUsage: false })
