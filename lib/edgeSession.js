...
import { canonicalizeSessionUser } from './accountIdentities'
...
const OAUTH_INTENT_COOKIE = 'tuaran_oauth_intent'
const ADMIN_HOST = 'admin.2aran.com'
const BOOKMARKS_HOST = 'bookmarks.2aran.com'
const CANONICAL_SESSION_PROXY_HOSTS = new Set([ADMIN_HOST, BOOKMARKS_HOST])
const CANONICAL_ME_URL = 'https://2aran.com/api/me'
const LAST_LOGIN_METHOD_MAX_AGE = 365 * 24 * 60 * 60
const LOGIN_METHODS = new Set(['credential', 'email', 'github', 'google', 'wechat'])
...
export async function getUserFromRequest(req) {
  ...
  return canonicalizeSessionUser(payload?.user || null)
}

export async function getUserFromCanonicalSession(cookieHeader, host) {
  if (!CANONICAL_SESSION_PROXY_HOSTS.has((host || '').split(':')[0].toLowerCase())) return null
...
export function serializeLastLoginMethodCookie(method, options = {}) {
  const value = String(method || '').toLowerCase()
  if (!LOGIN_METHODS.has(value)) return null
  const secure = options.secure ?? cookiesConfig().secure
  return serializeCookie(cookieNames.lastLoginMethod, value, {
...
export function getSecrets() {
  ...
  return {
    ...
    wechatAppId: env.WECHAT_APP_ID || process.env.WECHAT_APP_ID,
    wechatAppSecret: env.WECHAT_APP_SECRET || process.env.WECHAT_APP_SECRET,
    wechatLoginEnabled: (env.WECHAT_LOGIN_ENABLED || process.env.WECHAT_LOGIN_ENABLED || '').toLowerCase() === 'true',
...
export const cookieNames = {
  ...
  oauthIntent: OAUTH_INTENT_COOKIE,
  returnTo: RETURN_TO_COOKIE,
  guest: GUEST_COOKIE,
  lastLoginMethod: LAST_LOGIN_METHOD_COOKIE,
}
