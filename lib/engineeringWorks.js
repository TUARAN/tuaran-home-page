/**
 * 多维页面：自研可视化、亲身实测、长期写作项目、富数据分析。
 *
 * 页面结合作者判断、公开资料核验、交互设计与工程实现。
 *
 * 若先在 Cursor Canvas 里做交互原型，登记 canvasId（见 lib/canvasProvenance.js）。
 * `reviewReady: true` 代表页面已经完成人工原创性、证据、来源与政策风险复核，
 * 只控制本轮 AdSense 广告脚本范围，不影响 sitemap、公开搜索索引或站内发现。
 *
 * 既为页面壳、SEO 与统计提供注册信息，也给互动专题和内容目录复用。
 * 长期页面有实质内容更新时，补 `updated`（日期或带时区时间）；
 * SEO 的 dateModified 与 RSS 的版本化更新通知都会使用它。
 */

export const ENGINEERING_WORK_CATEGORIES = [
  {
    id: 'learning-tool',
    title: '学习与测验',
    description: '把题库、知识点和练习过程做成可操作的学习工具。',
  },
  {
    id: 'ai-engineering',
    title: 'AI 工程',
    description: '端侧推理、智能体能力、AI 工具链实验。',
  },
  {
    id: 'data-visualization',
    title: '数据可视化',
    description: '把复杂数据做成交互视图和可复盘判断。',
  },
  {
    id: 'engineering-research',
    title: '工程分析',
    description: '结合作者经验、公开资料与询证，分析技术路线、平台格局和真实成本。',
  },
  {
    id: 'long-term-project',
    title: '长期项目',
    description: '用页面承载持续写作、长期输入和阶段性产出。',
  },
  {
    id: 'life-system',
    title: '生活系统',
    description: '把家庭节奏与日常决策做成可持续使用的交互页面。',
  },
]

// 多维页面只允许两种展示模式，避免每个专题继续发明自己的页面壳、宽度和导航规则。
// site：跟随主站（站点导航 + 页脚 + 1120px 内容宽度 + 站点主题）。
// feature：沉浸式专题（隐藏主站 chrome + 全宽画布 + 深色专题基底）。
export const RICH_PAGE_PRESENTATIONS = Object.freeze({
  site: Object.freeze({
    id: 'site',
    label: '站点型',
    description: '与主站保持一致',
  }),
  feature: Object.freeze({
    id: 'feature',
    label: '沉浸型',
    description: '统一的独立专题样式',
  }),
})

export const ENGINEERING_WORKS = [
  {
    id: 'zhuangyuan',
    reviewReady: true,
    presentation: 'feature',
    category: 'data-visualization',
    subjects: ['humanities_history'],
    title: '中国历代状元：科年、原文与释义',
    summary:
      '从唐初进士第一到 1904 年甲辰科，收录可核对的状元档案、殿试对策原文与白话释义，并标明三元、除名、名次争议和末科传闻的证据边界。',
    date: '2026-09-25',
    href: '/zhuangyuan',
    kind: '科举文献档案',
    tags: ['状元', '科举', '殿试', '文天祥', '三元', '刘春霖', '中国历史'],
    badge: 'New',
  },
  {
    id: 'ming-emperors',
    reviewReady: true,
    presentation: 'feature',
    category: 'data-visualization',
    subjects: ['humanities_history'],
    title: '明朝十六帝：276 年皇权时间线',
    summary:
      '从洪武到崇祯，把 16 位皇帝、17 次登基、15 个关键事件放进同一条时间轴；可按历史阶段筛选、展开帝王档案、选择四人横向对比，并区分正史事实、后世推测与未证传闻。',
    date: '2026-09-25',
    href: '/ming-emperors',
    kind: '交互历史图谱',
    tags: ['明朝', '明朝十六帝', '中国历史', '皇帝', '时间线', '历史传闻'],
    badge: 'New',
  },
  {
    id: 'x-automation-retrospective',
    category: 'engineering-research',
    subjects: ['ai_dev'],
    title: 'X 自动发推：51 天回溯',
    summary:
      '从 2026-08-04 单条早安到五条短帖矩阵的完整演进：时间线、排期收缩、每日发帖量与 API 成本，含 9/18 去模板化与 9/23 暂停状态。',
    date: '2026-09-23',
    updated: '2026-09-23',
    href: '/x-automation-retrospective',
    kind: '自动化项目回溯',
    tags: ['X 平台', '自动发布', 'DeepSeek', 'GitHub Actions', '工程复盘'],
    badge: 'New',
  },
  {
    id: 'codex-reset',
    primaryDirectory: 'tools',
    category: 'data-visualization',
    subjects: ['ai_dev'],
    title: 'Codex 重置监控',
    summary:
      '按北京时间查看 Tibo 公开的 Codex 额度重置、重置卡发放与原帖；黄色是预告，绿色是已确认，不猜测下一次重置，也不读取个人额度。',
    date: '2026-09-22',
    updated: '2026-09-22',
    href: '/codex-reset',
    kind: '公开重置日历',
    tags: ['Codex', 'Tibo', '额度重置', '重置卡', 'AIHOT'],
    badge: 'New',
  },
  {
    id: 'workbuddy-hardware-access',
    primaryDirectory: 'tools',
    presentation: 'feature',
    category: 'ai-engineering',
    subjects: ['workbuddy'],
    title: 'WorkBuddy 硬件接入：四个接口跑通问与查',
    summary:
      '把 OAuth 授权、云端任务、PC 本地助理、结果回传与 macOS / Windows 内测安装包放进同一张交互地图；可切换查看问云端、查云端、问本地、查本地的 Scope、接口和返回方式。',
    date: '2026-09-20',
    href: '/tools/workbuddy-acp-bridge',
    kind: '硬件 Open API 互动专题',
    tags: ['WorkBuddy', '硬件接入', 'OAuth', 'Open API', '本地助理'],
    badge: 'New',
  },
  {
    id: 'whale-wallets',
    category: 'data-visualization',
    subjects: ['web3'],
    title: '公开巨鲸钱包',
    summary:
      '带公开标签和公众看法的 Bitcoin / Ethereum 大额地址：中本聪、赵长鹏、Vitalik 等归因，链上现读余额、折合美元、由近期交易估算的 24 小时变动。',
    date: '2026-09-12',
    updated: '2026-09-12',
    href: '/whale-wallets',
    kind: '链上数据可视化',
    tags: ['比特币', '以太坊', '巨鲸', '链上数据', '中本聪', '交易所冷钱包'],
    badge: 'New',
  },
  {
    id: 'ai-startup-survivor',
    reviewReady: true,
    presentation: 'feature',
    category: 'ai-engineering',
    subjects: ['ai_dev'],
    title: 'AI 创业幸存者',
    summary:
      '扮演独立开发者，在像素办公室里自动发射代码、清理不断出现的 Bug、收集算力并组合升级，坚持 60 秒完成产品发布。',
    date: '2026-08-27',
    href: '/ai-startup-survivor',
    kind: '像素生存网页游戏',
    tags: ['网页游戏', '像素风', '独立开发者', 'AI 创业', 'Canvas'],
    badge: 'New',
  },
  {
    id: 'quiz',
    category: 'learning-tool',
    subjects: ['humanities_history'],
    title: '2026 年党建知识题库',
    summary:
      '完整收录 100 道单选题与 1 道入党誓词填空题，提供即时反馈的学习模式，以及随机抽题、限时作答和错题复盘的考试模式。',
    date: '2026-08-18',
    href: '/quiz',
    kind: '党建知识测验',
    tags: ['党建知识', '题库', '学习模式', '考试模式'],
    badge: 'New',
  },
  {
    id: 'adsense-content-check',
    category: 'engineering-research',
    subjects: ['content_creation'],
    title: 'AdSense 内容质量自检',
    summary:
      '把 Google 的最低内容、原创性、内容贫乏与搜索垃圾政策转成一张可逐项判断的站点检查表；按阻断项、待确认项和已通过项生成当前整改清单。',
    date: '2026-08-10',
    href: '/adsense-content-check',
    kind: '内容质量自检',
    badge: 'New',
  },
  {
    id: 'margin-account-313m-loss-investigation',
    category: 'engineering-research',
    subjects: ['business_market'],
    title: '月亏3.13亿：一张两融截图背后的十亿级资金谜局',
    summary:
      '从午盘指数、9.23亿元隐含期初净资产和两套杠杆情景出发，把行情真实、账户数据真实与账户身份真实分成三层核验；追查“神秘游资”传闻的证据缺口，并评估截图修改、借图传播与多账户拼接等可能性。',
    date: '2026-07-21',
    href: '/margin-account-313m-loss-investigation',
    kind: '财经截图调查',
    badge: 'New',
  },
  {
    id: 'ai-agent-communications-industry-report',
    reviewReady: true,
    category: 'engineering-research',
    subjects: ['ai_dev'],
    title: 'AI 智能体通信能力行业报告',
    summary:
      '从 OpenClaw Channel 与通信 Skill、WorkBuddy、豆包、微信“小微”到 Token 经济，区分官方数据、第三方监测和本站测算，解释消息、短信与电话如何被封装为 Agent 可执行能力。',
    date: '2026-07-21',
    href: '/ai-agent-communications-industry-report',
    kind: '前沿行业研究',
    badge: 'New',
  },
  {
    id: 'x-platform-intelligence',
    reviewReady: true,
    category: 'data-visualization',
    subjects: ['content_creation'],
    title: 'X 值不值得做？创作者经营情报',
    summary: '为中文科技创作者回答 X 是否值得投入、应该负责什么、怎样形成经营闭环；把行动建议与用户规模、市场信号、平台风险和完整证据放在同一条决策路径里。',
    date: '2026-07-20',
    href: '/x-platform-intelligence',
    kind: '创作者经营指南',
    badge: 'New',
  },
  {
    id: 'wisdom-frontier',
    reviewReady: true,
    pvKey: 'resource/wisdom-frontier',
    category: 'data-visualization',
    subjects: ['humanities_history'],
    title: '智慧边界：全球顶级奖项与人类成就图谱',
    summary:
      '覆盖自然科学、数学、计算机、工程、医学、设计、人文与艺术等 15 个领域，以 33 项全球顶级奖项、代表人物和关键成就建立可筛选、可持续扩充的智慧地图。',
    date: '2026-07-18',
    href: '/wisdom-frontier',
    kind: '人类成就图谱',
    badge: 'New',
  },
  {
    id: 'guoqi-guodan',
    presentation: 'feature',
    category: 'engineering-research',
    subjects: ['business_market'],
    title: '国企“过单”：走单、空转贸易与融资性贸易全流程',
    summary:
      '从交易实质出发，拆解融资垫资、冲量空转与真实供应链三类场景；把双方动机、货权控制、资金路径、发票边界、四层风险和七项红旗做成可切换、可自查的合规专题页。',
    date: '2026-07-18',
    href: '/guoqi-guodan',
    kind: '贸易合规专题',
    badge: 'New',
  },
  {
    id: 'global-ai-governance',
    category: 'engineering-research',
    subjects: ['humanities_history'],
    title: '全球 AI 治理平台与机制全景图',
    summary:
      '把 GPAI、OECD.AI、联合国体系、WAICO、上合、非盟、金砖、东盟等 14 个国际 AI 组织、政治进程、战略与研究网络放进同一张制度地图；可按阵营、权威和执行力筛选，展开档案并做最多四项横向对比。',
    date: '2026-07-17',
    href: '/global-ai-governance',
    kind: '全球治理图谱',
    badge: 'New',
  },
  {
    id: 'workbuddy-harness',
    reviewReady: true,
    category: 'ai-engineering',
    subjects: ['workbuddy'],
    title: 'WorkBuddy Harness：给 AI Agent 补上一套“运行制度”',
    summary:
      '从技术博主视角拆解九维 Agent 基础设施：Engine 如何驱动 Hooks，身份、记忆、调度、安全、评测与多 Agent 协作如何进入同一套运行闭环，以及项目真正的价值和当前边界。',
    date: '2026-07-16',
    href: '/workbuddy-harness',
    kind: '开源项目拆解',
    badge: 'New',
  },
  {
    id: 'network-access-guide',
    category: 'engineering-research',
    subjects: ['web_cloud'],
    title: '5 个网络加速服务公开信息核验',
    summary:
      '对红海 Pro、平行网、脉动云、火烧云、鱼云的当前状态、套餐、协议、节点与来源做横向核验；把官网事实、第三方口径和待验证项拆开，给出月付实测与购买前检查框架。',
    date: '2026-07-15',
    href: '/network-access-guide',
    kind: '公开信息核验',
    badge: 'New',
  },
  {
    id: 'xiaomoli-dad-todo',
    category: 'life-system',
    subjects: ['life_family'],
    audience: 'owner',
    title: '小茉莉的爸爸带娃清单',
    summary:
      '围绕日常、习惯与家庭节奏的私域打卡页面：按日完成清单、回看月历与 30 天统计，把零散的带娃事项沉淀为可持续复盘的家庭系统。',
    date: '2026-07-12',
    href: '/xiaomoli-dad-todo',
    kind: '家庭生活系统',
    badge: 'New',
  },
  {
    id: 'eatwhat',
    category: 'life-system',
    subjects: ['life_family'],
    title: '吃什么',
    summary:
      '把一日三餐的选择做成家庭决策页：根据时段随机推荐爸爸妈妈的菜单，并单独维护适合小茉莉的宝宝餐清单。',
    date: '2026-07-12',
    href: '/eatwhat',
    kind: '家庭决策页面',
    badge: 'New',
  },
  {
    id: 'web-llm',
    primaryDirectory: 'tools',
    reviewReady: true,
    presentation: 'feature',
    category: 'ai-engineering',
    subjects: ['ai_dev'],
    title: '端侧大模型实验台',
    summary:
      '浏览器端运行大模型的工程实验：围绕 WebGPU、Transformers.js、ONNX Runtime、Ollama 与边缘设备，把端侧推理从概念验证落到可访问、可体验的站内页面。',
    date: '2026-06-03',
    href: '/web-llm',
    kind: '端侧 AI 工程',
    badge: 'New',
  },
  {
    id: 'skill-market-research',
    reviewReady: true,
    category: 'engineering-research',
    subjects: ['ai_dev'],
    title: 'Skill 上架、宣发与回流工程分析',
    summary:
      '从 Codex / GitHub / ClawHub / X / 第三方 marketplace 观察主流 Skill 的制作、包装、上架、宣发与回流方式，整理成「可执行能力 → 可信开源项目 → 可安装市场 → 社交传播 → 用户回流」的工程打法。',
    date: '2026-06-05',
    href: '/skill-market-research',
    kind: '多维分析',
    badge: 'New',
  },
  {
    id: 'platform-framework-pairs',
    reviewReady: true,
    category: 'engineering-research',
    subjects: ['web_cloud'],
    title: '《Anthropic × Bun + Cloudflare × VoidZero + Vercel × Next：三极割据，AI 公司直接下场抢 Web Runtime》',
    summary:
      '2025-12 Anthropic 收购 Bun（公司首次收购）+ 2026-06 Cloudflare 收购 VoidZero + 2016 起 Vercel × Next：三极格局成形。Anthropic 是新形态 —— AI 模型公司直接拥有 runtime，绕过 deployment 平台层。10 节研报框架 + 11 组「平台 × 框架」配对可视化（散点 / 排行 / 焦点 / 对比 / 全部可分享）。',
    date: '2026-06-05',
    href: '/platform-framework-pairs',
    kind: '富数据研判',
    badge: 'Featured',
  },
  {
    id: 'cancers-overview',
    category: 'data-visualization',
    subjects: ['life_family'],
    title: '癌症全景',
    summary:
      '10 种主要癌症的发病、死亡、5 年生存、性别 / 年龄分布与风险因子可视化：四象限散点 + 横向排行 + 焦点 / 对比模式 + 全球 / 中国双口径切换；数据来自 GLOBOCAN 2022 与 NCC 2024。',
    date: '2026-06-04',
    href: '/cancers-overview',
    kind: '富数据可视化',
    badge: 'New',
  },
  {
    id: 'tang-ping-map',
    presentation: 'feature',
    category: 'data-visualization',
    subjects: ['life_family'],
    title: '躺平地图：低总价房源多维观察',
    summary:
      '基于 Tang Ping Map 公开点位做站内多维页面：121 个低总价房源样本，按省份、关键词、总价、面积、租金、租金回报和回本周期筛选观察，并用经纬度近似投影呈现地理分布。',
    date: '2026-07-04',
    href: '/tang-ping-map',
    kind: '地图数据可视化',
    badge: 'New',
  },
  {
    id: 'x-mutual-aid-circle',
    primaryDirectory: 'community',
    pvKey: 'resource/x-mutual-aid-circle',
    presentation: 'feature',
    category: 'data-visualization',
    subjects: ['content_creation'],
    title: 'X 互帮互助圈子：真实互动，一起把 X 流量玩明白',
    summary:
      'X 早期互动、互关清理、发帖时段热力图、Tweepcred 评分与可见性检测合在一起的社群增长多维页面：把圈子规则、工具链和时间策略放到同一个可操作入口。',
    date: '2026-07-03',
    href: '/x-mutual-aid-circle',
    kind: '社群增长工具页',
    badge: 'Hot',
  },
  {
    id: 'sun-moon-motion',
    category: 'data-visualization',
    subjects: ['humanities_history'],
    title: '日月运行交互可视化',
    summary:
      '用日心视角探索太阳中心、地球公转与自转、月球绕地运行与月相变化；把日出日落、昼夜分界和月相循环放在一个可交互模型里复盘。',
    date: '2026-05-31',
    href: '/sun-moon-motion',
    kind: '交互可视化',
  },
  {
    id: 'ai-token-usage-research',
    reviewReady: true,
    category: 'engineering-research',
    subjects: ['ai_dev'],
    title: 'AI Token 用量与花费强度分析',
    summary:
      '日耗 1 亿 / 4.5 亿 tokens（账单口径，含缓存命中）对照：账单 vs 净处理双账户 + 对数刻度强度尺 + cache-aware 三段定价折算月费 + 订阅 vs 按量口径 + 效率信号 + 优化抓手 ROI 排序。',
    date: '2026-05-31',
    href: '/ai-token-usage-research',
    kind: '实测数据',
    canvasId: 'ai-token-usage-analysis',
  },
  {
    id: 'zhang-juzheng-book',
    reviewReady: true,
    category: 'long-term-project',
    subjects: ['content_creation'],
    title: '《张居正：一个改革者的成事与代价》· 写作出版工程',
    summary:
      '用输出倒逼输入：把"写一本张居正的书并发布出版"作为长期富页面项目运营。主线、目录、人物关系、关键事件、12 个月节奏与进度看板。',
    date: '2026-05-30',
    href: '/zhang-juzheng-book',
    kind: '长期项目',
  },
]

export function getRichPagePresentation(work) {
  const presentation = work?.presentation || 'site'
  return RICH_PAGE_PRESENTATIONS[presentation] || RICH_PAGE_PRESENTATIONS.site
}

/**
 * ENGINEERING_WORKS 同时承担页面壳、SEO 与阅读统计注册，不能等同于公开目录归属。
 * 只有主归属为互动专题的页面才进入 /rich-pages、内容目录和对应导航。
 */
export const INTERACTIVE_DIRECTORY_WORKS = ENGINEERING_WORKS.filter(
  (work) => (work.primaryDirectory || 'interactive') === 'interactive',
)

export function getRichPageByPath(pathname) {
  return ENGINEERING_WORKS.find((work) => work.href === pathname) || null
}

export function getRichPagePvKey(work) {
  return work?.pvKey || (work?.id ? `rich-page/${work.id}` : '')
}
