...
import { displayNameForUserId } from './userDisplayName'
import { migrateGuestUnlockLedger } from './points'
import { migrateGuestResourceEvents } from './resourceEvents'
import {
  cookieNames,
  cookiesConfig,
  getSecrets,
...
/** 游客展示名：趣味昵称（形容词+动物+短哈希），稳定、不泄露完整 id */
export function guestDisplayName(gid) {
  const u = displayNameForUserId(`${GUEST_USER_PREFIX}${gid}`)
  return `${u.emoji} ${u.name} ${u.short}`
}
  ...
  try {
    ...
    await db.batch([
      ...
      db
        ...
        .bind(userId, provider, guestId),
      // 把游客已解锁的调研文章迁到正式账号，绑定后仍免费可读（冲突的已解锁项忽略）
      db
        .prepare(
          `UPDATE OR IGNORE resource_unlocks
             SET user_id = ?1
           WHERE user_id = ?2`
        )
        .bind(userId, guestId),
    ])
    await migrateGuestUnlockLedger(db, guestId, userId).catch((error) => {
      console.error('migrateGuestUnlockLedger failed', error)
    })
    await migrateGuestResourceEvents(db, guestId, userId).catch((error) => {
      console.error('migrateGuestResourceEvents failed', error)
    })
    return gid
  } catch (error) {
