...
/**
 * 首屏批次先按空置顶画出来。后台设置到达后，只把人工置顶挪到最前，其余条目保持原顺序。
 */
export function reconcilePaintedHomeRecommendationPins(items, catalog, rawSettings) {
  if (!Array.isArray(items) || !items.length) return items || []
  const settings = mergeHomeRecommendationSettings(rawSettings)
  const byId = new Map()
  for (const item of [...(Array.isArray(catalog) ? catalog : []), ...items]) {
    if (item?.id) byId.set(item.id, item)
  }
  const pinned = settings.pinnedIds.map((id) => byId.get(id)).filter(Boolean)
  const pinnedIds = new Set(pinned.map((item) => item.id))
  const rest = items.filter((item) => !pinnedIds.has(item.id))
  const next = [...pinned, ...rest].slice(0, Math.max(items.length, pinned.length))
  if (next.length === items.length && next.every((item, index) => item.id === items[index]?.id)) return items
  return next
}
