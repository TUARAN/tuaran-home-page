...
/**
 * 从完整候选池选出一批推荐。固定项和最新项优先；其余按权重随机或时间轮换，
 * 在候选数量足够时不会重复上一批的普通内容。
 */
export function chooseHomeRecommendationBatch(catalog, rawSettings, batchNumber, previousIds = []) {
  ...
  const remainingSlots = Math.max(0, settings.batchSize - selected.length)

  const latest = [...eligible]
    .filter((item) => !selectedIds.has(item.id))
    .sort((a, b) => String(b.sortKey || '').localeCompare(String(a.sortKey || '')))
    .slice(0, Math.min(settings.latestCount, remainingSlots))
    .map((item) => ({ ...item, isLatest: true }))
  latest.forEach((item) => selectedIds.add(item.id))
  selected.push(...latest)

  const oldIds = new Set(previousIds)
  const pool = eligible.filter((item) => !selectedIds.has(item.id))
  ...
  if (settings.rotationMode === 'ordered') {
    ...
    const seed = `home-recommendations:${batchNumber}`
