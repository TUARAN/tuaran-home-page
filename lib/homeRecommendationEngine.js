...
/** 首页文章栏的一级范围：推荐沿用批次，最新按时间，资源只列资源条目。 */
...
function sameRecommendationItem(left, right) {
  return Boolean(left)
    && Boolean(right)
    && left.id === right.id
    && Boolean(left.isLatest) === Boolean(right.isLatest)
    && recommendationItemSignature(left) === recommendationItemSignature(right)
}
...
/**
 * 未读到运行时目录前不标「最新」。读到之后也只给已经出现在当前列表、且确实最新的条目打标，不改列表顺序。
 */
export function tagVisibleHomeRecommendationLatest(items, catalog, rawSettings, runtimeReady) {
  if (!Array.isArray(items) || !items.length) return items || []
  const latestId = runtimeReady ? pickLatestHomeRecommendationItem(catalog, rawSettings)?.id : null
  const tagged = items.map((item) => {
    const isLatest = Boolean(latestId) && item.id === latestId
    return item.isLatest === isLatest ? item : { ...item, isLatest }
  })
  const unchanged = tagged.length === items.length
    && tagged.every((item, index) => sameRecommendationItem(item, items[index]))
  return unchanged ? items : tagged
}

/**
 * 运行时目录可能比构建期首屏更新。只替换原来的「最新」位，避免整批内容重排。
 */
