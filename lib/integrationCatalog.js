...
export const INTEGRATION_SERVICES = [
  ...
  },
  {
    ...
    purpose: 'Git 调研审批发布、A 股与加密草稿发布、本站开发同步、Autopilot 巡检写回。',
  ...
  },
  {
    ...
    managedIn: '/admin/content-weekly',
    note: '数据统计只需要 Account Analytics Read 的只读 Token；Zone ID 不是密钥，但需配置以限定站点。',
  },
  {
    id: 'umami',
    label: 'Umami Cloud',
    provider: 'Umami Cloud Analytics API',
    purpose: '站点访客、访问次数、页面浏览、跳出率与访问时长的主口径。',
    envRefs: ['UMAMI_API_KEY', 'UMAMI_WEBSITE_ID'],
    managedIn: '/admin/content-weekly',
    note: 'API Key 仅服务端读取；UMAMI_WEBSITE_ID 未配置时使用前台埋点的当前网站 ID。',
  },
  {
    id: 'coingecko',
    label: 'CoinGecko',
  ...
  {
    id: 'content-ledger-ipfs',
    label: '内容账本 IPFS',
    provider: 'Pinata',
    purpose: '精选公开文章的 IPFS pinning；回读必须走与 Pinata 网关分离的双网关。',
    envRefs: ['PINATA_JWT'],
    note: '只 pin 规范化副本字节。JWT 不进仓库、D1 或公开 proof JSON。',
  },
  {
    id: 'content-ledger-publisher',
    label: '内容账本发布钱包',
    provider: 'Base / EAS',
    purpose: '内容存证专用发布钱包：测试网与主网密钥隔离，只签名 attestation。',
    envRefs: [
      'CONTENT_LEDGER_TESTNET_PRIVATE_KEY',
      'CONTENT_LEDGER_MAINNET_PRIVATE_KEY',
      'CONTENT_LEDGER_PUBLISHER_ADDRESS',
      'CONTENT_LEDGER_FORBIDDEN_ADDRESSES',
    ],
    note: '主网必须 --confirm-mainnet，且不得复用测试网、站长资金或读者钱包。',
  },
]
...
export const INTEGRATION_WEBHOOKS = [
  ...
  {
    id: 'rss-updates',
    path: 'POST /api/cron/rss-updates',
    purpose: '检查后台收录的 RSS 源，把新条目写入站长通知中心。',
    secretEnv: 'RSS_UPDATES_SECRET（回退 WEEKLY_SUMMARY_SECRET / PUBLIC_OPINION_COLLECT_SECRET）',
    workflow: 'https://github.com/TUARAN/tuaran-home-page/actions/workflows/rss-updates.yml',
  },
  {
    id: 'weekly-summary',
  ...
  },
  {
    ...
    workflow: 'GitHub Actions 失败步骤与 pages-deploy-alert 工作流直接调用',
  },
]
