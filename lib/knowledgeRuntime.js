...
async function researchOverridesOrEmpty() {
  try {
    return await listResearchOverrides()
  } catch (error) {
    if (String(error?.message || '').includes('D1 binding DB is missing')) return []
    throw error
  }
}
...
export async function readRuntimeKnowledgeItems() {
  const [catalog, research, posts] = await Promise.all([
    readContentCatalog(), researchOverridesOrEmpty(), listDiscoverablePosts({ includeStaticOverrides: true }),
  ])
