...
import { buildOllamaAuthHeaders, buildOllamaChatRequest, normalizeOllamaModels, ollamaNativeChatUrl, ollamaTagsUrl, parseOllamaChatResponse } from './ollamaCore.js'
...
export async function listOllamaModels(providerId, { timeoutMs = 12000 } = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      throw codedError('OLLAMA_MODELS_API_FAILED', data?.error?.message || data?.error || `Ollama 模型列表返回 HTTP ${response.status}`, { status: response.status })
    }
    try {
