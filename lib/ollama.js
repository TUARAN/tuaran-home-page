...
import { ollamaChatUrl, parseOllamaChatResponse } from './ollamaCore.js'
...
export async function getOllamaProvider(id, { requireActive = true } = {}) {
  ...
  if (row.auth_cipher) {
    ...
    token = await decryptApiKey(row.auth_cipher, secret)
  }
  return { row, token }
}
...
export async function callOllama({
  ...
  const { row, token } = await getOllamaProvider(providerId)
  ...
  try {
    const response = await fetch(ollamaChatUrl(row.base_url), {
      method: 'POST',
      ...
      body: JSON.stringify({ model: resolvedModel, messages, temperature, max_tokens: maxTokens, stream: false }),
    })
