...
export function ollamaUpstreamError(status, data) {
  const upstreamStatus = Math.max(0, Number(status) || 0)
  ...
  return {
    ...
    message: data?.error?.message || data?.error || `Ollama 返回 HTTP ${upstreamStatus || '未知'}`,
...
function modelDisplayName(name, parameterSize, quantizationLevel) {
  ...
  // Ollama reports the exact tensor count (for example 9.7B / 26.9B), while
  // model tags carry the familiar marketed size (9B / 27B). Keep the exact
  // value in parameterSize, but use the tag size in the human-facing label.
  const declaredSizeMatch = String(name || '').match(/(?:^|[:-])(\d+(?:\.\d+)?)b(?=[:-]|$)/i)
  const displayParameterSize = declaredSizeMatch ? `${declaredSizeMatch[1]}B` : parameterSize
  const modelAndSize = [family, displayParameterSize].filter(Boolean).join(' ')
