...
async function validateProfileReference(db, directionId, projectId) {
...
export async function createManualEvent(db, payload, context = {}) {
  ...
  if (!await eventTargetExists(db, payload.entityType, payload.entityId)) return resultError('ENTITY_NOT_FOUND')
...
export async function updateProjectProfile(db, id, changesToRead, context = {}) {
  ...
  if (invalidProject) return invalidProject
  const invalidDirection = await validateDirectionReference(db, candidate.directionId)
  if (invalidDirection) return invalidDirection
...
function versionedUpdateStatement(db, table, id, changesToWrite, updatedAt, expectedUpdatedAt, guardOpenTasks) {
  const entries = Object.entries(changesToWrite)
  const assignments = entries.map(([column], index) => `${column} = ?${index + 1}`)
  assignments.push(`updated_at = ?${entries.length + 1}`)
  const idIndex = entries.length + 2
  const versionIndex = entries.length + 3
  const milestoneGuard = guardOpenTasks
    ? ` AND NOT EXISTS (
  ...
  return db.prepare(`UPDATE ${table}
    ...
    WHERE id = ?${idIndex} AND updated_at = ?${versionIndex}${milestoneGuard}`)
    .bind(...entries.map(([, value]) => value), updatedAt, id, expectedUpdatedAt)
}
...
async function updateWorkItem(db, entity, table, mapper, id, changesToRead, columns, context) {
  ...
  const invalidParent = entity === 'milestone'
    ? await validateProfileReference(db, candidate.directionId, candidate.projectId)
    : await validateMilestoneReference(db, candidate.milestoneId)
  ...
  const statement = versionedUpdateStatement(db, table, id, selected, updatedAt, before.updatedAt, completingMilestone)
