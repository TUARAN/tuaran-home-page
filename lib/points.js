...
export const POINT_POLICY = {
  ...
  earnMethods: [
    ...
    },
    {
      ...
      status: 'live',
      description: `登录用户可在余额组件签到，按 UTC 自然日幂等发放；邮箱未激活前最多先签到 ${POINT_RULES.pendingCheckinLimit} 次。`,
    },
    {
      id: 'comment',
      label: '有效评论',
  ...
  spendScenarios: [
    ...
    },
    {
      id: 'tool_claim',
      label: '工具包 / 安装包领取',
      cost: POINT_RULES.toolDefaultCost,
      unit: '项',
      status: 'live',
    ...
    },
    {
      ...
      description: '登录用户上传图片到站内 R2 图床，每张图片消耗 5 燃币。',
    },
    {
      id: 'content_boost',
      label: '内容 Boost / 推荐',
...
// 资源 key 前缀 → 默认额度：文字内容统一 5；工具包使用 10。
const DEFAULT_COST_BY_PREFIX = [
  ...
  { re: /^tool:/, cost: () => POINT_RULES.toolDefaultCost },
]

/**
 * 取资源权益配置：优先 gated_resources 里的显式配置；
 * 调研 / 资料类 key 若未显式配置，按前缀回退到默认额度，
 * 从而实现「文字内容默认 5、工具包默认 10」，无需逐条登记。
 */
function defaultResourceFor(resourceKey) {
  const catalog = getResourceCatalogItem(resourceKey)
  if (catalog?.defaultCost != null) {
    return {
      resource_key: resourceKey,
      cost_points: Math.max(0, Math.trunc(Number(catalog.defaultCost) || 0)),
      min_role: 'guest',
      synthetic: true,
    }
  }
...
export async function spendPoints(db, userId, { cost, reason, ref, now = Date.now() }) {
  ...
  const inserted = await db
    ...
    .bind(id, -amount, spendReason, spendRef, now)
    .run()

  if (!(Number(inserted?.meta?.changes || 0) > 0)) {
    return { ok: true, alreadySpent: true, cost: amount, balance: await getBalance(db, id) }
  }
  ...
  return { ok: true, cost: amount, balance: Number(row?.balance || 0) }
}
...
async function missingUnlockLedgerRollup(db, userId) {
  const id = String(userId || '').trim()
  ...
  const row = await db
    ...
    .bind(id, POINT_RULES.researchDefaultCost, POINT_RULES.resourceDefaultCost)
    .first()
  return {
...
/**
 * 给已有 resource_unlocks 补齐缺失的 unlock 负流水，并用账本重算余额。
 * 这是为了修复旧版本「游客解锁权益迁移了，但扣款流水留在 guest 账号」造成的错位。
 */
export async function reconcileUnlockLedgerForUser(db, userId, now = Date.now()) {
  ...
  const before = await missingUnlockLedgerRollup(db, id)
  if (before.count > 0) {
    await db
      .prepare(
        `WITH unlock_costs AS (
  ...
  const balance = await repairBalanceFromLedger(db, id, now)
  const after = await missingUnlockLedgerRollup(db, id)
  return {
    ok: true,
...
export async function reconcileUnlockLedgerForAccounts(db, { limit = 500 } = {}, now = Date.now()) {
  ...
  const candidates = await db
    ...
    .bind(POINT_RULES.researchDefaultCost, POINT_RULES.resourceDefaultCost, max)
    .all()
...
export async function migrateGuestUnlockLedger(db, guestUserId, userId, now = Date.now()) {
  ...
  await db
    .prepare(
      `INSERT OR IGNORE INTO point_ledger (user_id, delta, reason, ref, created_at)
       SELECT ?2, gl.delta, gl.reason, gl.ref, gl.created_at
         FROM point_ledger gl
        WHERE gl.user_id = ?1
          AND gl.reason = 'unlock'
          AND gl.delta < 0`
    )
    .bind(guestId, id)
    .run()
...
export async function unlockResource(db, userId, resourceKey, now = Date.now()) {
  ...
  if (!(Number(ledger?.meta?.changes || 0) > 0)) {
    await db
      .prepare(
        `INSERT OR IGNORE INTO resource_unlocks (user_id, resource_key, unlocked_at, cost_points)
         VALUES (?1, ?2, ?3, ?4)`
