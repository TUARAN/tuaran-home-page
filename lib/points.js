...
// 调研文章资源 key 形如 research:<category>:<slug>，默认即受燃币墙保护
const RESEARCH_KEY_RE = /^research:/

/**
 * 取资源门槛配置：优先 gated_resources 里的显式配置；
 * 调研类 key 若未显式配置，回退到默认价（researchDefaultCost），
 * 从而实现「调研文章默认消耗 5 燃币」，无需逐篇登记。
 */
function defaultResourceFor(resourceKey) {
  if (RESEARCH_KEY_RE.test(resourceKey)) {
    return {
      resource_key: resourceKey,
      cost_points: POINT_RULES.researchDefaultCost,
      min_role: 'guest',
      synthetic: true,
    }
  }
