  ...
  return (Array.isArray(items) ? items : items ? [items] : [])
    .map((item) => {
      ...
      return {
        ...
        engagement: 10,
        text,
        sourceExcerpt: cleanText(item.description || item['content:encoded'], 700),
...
async function translatePostsWithDeepSeek(batch, env) {
  const result = await callDeepSeekJson({
    env,
    messages: buildLocalizationMessages(batch),
    ...
    task: {
      ...
      taskType: 'localize-feed',
      title: `舆情样本汉化（${batch.length} 条）`,
      actorId: 'public-opinion-collector',
      actorName: '舆情定时采集',
      inputSummary: batch.map((post) => post.text).join('\n').slice(0, 1200),
      metadata: { postIds: batch.map((post) => post.id) },
    },
  })
  return result.json
}
...
export async function runPublicOpinionCollect({ db, env, log = () => {} }) {
  ...
  const localization = await localizePublicOpinionPosts(
    uniquePosts,
    (batch) => translatePostsWithDeepSeek(batch, env),
  )
  log(`DeepSeek/localize: ${localization.translatedCount}/${uniquePosts.length}`)
  if (localization.failedBatches) {
    failures.push(`DeepSeek/localize: ${localization.errors.join('; ')}`)
  }

  await upsertPosts(db, localization.posts, collectedAt)
  await writeTrend(db, localization.posts, collectedAt)
  await db
  ...
  return {
    ...
    localizedPosts: localization.translatedCount,
    localizationFailedBatches: localization.failedBatches,
    sourcesSucceeded: results.length - sourceFailureCount,
    sourcesFailed: sourceFailureCount,
    failures,
