...
export async function runQuoteAutomation({ db, env, force = false } = {}) {
  ...
  const recentQuotes = (recentResult?.results || []).map((row) => row.text)
  const prompt = buildAutomatedQuotePrompt({ recentQuotes })
  ...
  const quote = await insertGeneratedQuote(db, parseGeneratedQuotes(result.content, { recentQuotes })[0], {
    prompt,
    trigger: 'automation',
