...
export const CONTENT_TYPE_META = {
  opinion: { label: '观点' },
  analysis: { label: '分析' },
  engineering_case: { label: '工程案例' },
  build_log: { label: '建站日志' },
  practice: { label: '实践' },
  guide: { label: '指南' },
...
// 技术内容二级分类（frontmatter: tech_type）
...
export const TECH_TYPE_META = {
  models_compute: { label: '大模型与算力', tone: 'violet' },
  agents_automation: { label: 'Agent 与自动化', tone: 'emerald' },
  ai_coding: { label: 'AI 编程与工具', tone: 'blue' },
  web_cloud: { label: 'Web 与云平台', tone: 'sky' },
  architecture: { label: '工程架构', tone: 'slate' },
  security_identity: { label: '安全与身份', tone: 'rose' },
  networking: { label: '网络与通信', tone: 'amber' },
  data_algorithms: { label: '数据与算法', tone: 'stone' },
}

export function getTechTypeFilters() {
  return [
    { key: 'all', label: '全部技术' },
    ...TECH_TYPES.map((key) => ({ key, label: TECH_TYPE_META[key].label })),
  ]
}
