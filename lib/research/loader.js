...
export const VARIANT_LABELS = {
  'claude-code': 'Claude Code（Opus 4.7）协助',
  cursor: 'Cursor 协助',
  doubao: '豆包协助',
  codex: 'Codex（GPT-5.5）协助',
  gemini: 'Gemini 协助',
  gpt: 'GPT 协助',
  manual: '手写',
}
...
function getResearchSourceLabel(source) {
  const id = String(source || '').toLowerCase()
  if (id === 'manual') return 'TUARAN'
  const assistant = VARIANT_LABELS[id] || (id ? `${id} 协助` : '大模型协助')
  return `TUARAN（${assistant}）`
}
...
function readEntry(category, filename) {
  ...
  const defaultVariantId = (data.source || 'claude-code').toLowerCase()
  ...
  const entry = {
    ...
    source: data.source || '',
    sourceLabel: getResearchSourceLabel(data.source),
    model: data.model || '',
