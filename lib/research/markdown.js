...
function collectTokenText(tokens = []) {
  return tokens
    .map((token) => {
      if (Array.isArray(token.tokens) && token.tokens.length) return collectTokenText(token.tokens)
      if (Array.isArray(token.items)) {
        return token.items.map((item) => collectTokenText(item.tokens || [])).join('')
      }
...
/** 边界说明标签。写法见 research/README.md「边界说明」。渲染时统一挪到文末。 */
export const COLLAPSIBLE_NOTE_LABEL_RE = /^(适用范围|风险与合规提示|风险提示|合规提示(?:[（(][^）)]*[）)])?)[：:]/

function escapeRegExp(value) {
  return String(value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
}

function stripRiskNoteLead(html, label) {
  const escaped = escapeRegExp(label)
  const lead = new RegExp(
    `^(\\s*<p>)(?:<strong>\\s*${escaped}\\s*[：:]\\s*</strong>|<strong>\\s*${escaped}\\s*</strong>\\s*[：:]|${escaped}\\s*[：:])\\s*`,
  )
  return html.replace(lead, '$1')
}

function renderRiskNote(label, body) {
  return [
    '<details class="research-risk-note research-risk-note--end" data-research-note-placement="end">',
    `<summary><span class="research-risk-note__label">${escapeHtml(label)}</span><span class="research-risk-note__toggle"><span class="research-risk-note__more">展开</span><span class="research-risk-note__less">收起</span></span></summary>`,
    `<div class="research-risk-note__body">${stripRiskNoteLead(body, label)}</div>`,
    '</details>\n',
  ].join('')
}

function relocateEndNotes(html) {
  const notes = []
  const remaining = String(html || '').replace(
    /<details class="[^"]*" data-research-note-placement="end">[\s\S]*?<\/details>\n*/g,
    (block) => {
      notes.push(block.trimEnd())
      return ''
    },
  )
...
function collectFenceRanges(lines) {
  const ranges = []
  let start = -1
...
function isIndexInsideFence(ranges, index) {
  return ranges.some(([start, end]) => index > start && index <= end)
}

function moveIndexOutOfFence(ranges, index, lineCount) {
  for (const [start, end] of ranges) {
    if (index > start && index <= end) return Math.min(lineCount, end + 1)
  }
  return index
}
  ...
  m.use({
    ...
    renderer: {
      ...
      blockquote({ tokens }) {
        ...
        const plain = collectTokenText(tokens).replace(/\s+/g, ' ').trim()
        const match = COLLAPSIBLE_NOTE_LABEL_RE.exec(plain)
        if (!match) return `<blockquote>\n${body}</blockquote>\n`
        return renderRiskNote(match[1], body)
      },
    },
  })
  return m
...
function insertResearchImages(markdown, images, options = {}, renderImage = renderFigure) {
  ...
  const fenceRanges = collectFenceRanges(lines)
  ...
  function rememberInsertion(index, image) {
    const insertAt = moveIndexOutOfFence(fenceRanges, index, lines.length)
  ...
  if (headingIndexes.length > 0) {
    ...
    for (let i = 0; i < imageCount; i += 1) {
      ...
      let walkingFence = false
      while (insertAt < lines.length) {
        const line = lines[insertAt]
        if (isFenceLine(line)) {
          walkingFence = !walkingFence
          insertAt += 1
          continue
        }
        if (walkingFence) {
      ...
      rememberInsertion(insertAt, images[i])
    }
  } else {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      if (isIndexInsideFence(fenceRanges, i)) continue
    ...
    if (points.length === 0) {
      ...
      for (let i = 0; i < imageCount; i += 1) {
        ...
        usedPoints.add(pi)
        rememberInsertion(points[pi], images[i])
      }
    }
  }
...
export function renderMarkdown(markdown, options = {}) {
  ...
  const withTables = html.replace(/<table>/g, '<div class="table-scroll"><table>').replace(/<\/table>/g, '</table></div>')
  return relocateEndNotes(withTables)
}
