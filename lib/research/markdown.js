    ...
    renderer: {
      ...
      link({ href, title, tokens }) {
        ...
        const extractAttrs = extractCode
          ? ` data-extract-code="${escapeAttribute(extractCode)}" class="inline-flex items-center rounded-lg border border-amber-400 bg-amber-50 px-2.5 py-1 font-semibold text-amber-800 no-underline shadow-sm transition hover:bg-amber-100 dark:border-amber-500/60 dark:bg-amber-950/40 dark:text-amber-200"`
          : ''
        const hint = extractCode
          ? '<span class="ml-1.5 text-[10px] font-medium opacity-70">复制码并打开 ↗</span>'
          : ''
        return `<a href="${escapeAttribute(safeHref)}"${titleAttr}${extra}${extractAttrs}>${text}${hint}</a>`
      },
