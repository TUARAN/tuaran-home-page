...
function renderFigure(image, title, index) {
  ...
  const caption = image.credit ? `${alt} · ${image.credit}` : alt
  return [
...
function insertResearchImages(markdown, images, options = {}) {
  ...
  for (let i = 0; i <= lines.length; i += 1) {
    if (insertionMap.has(i)) {
      ...
      out.push(...figures.map((image, index) => renderFigure(image, options.title, index)))
      out.push('')
    }
