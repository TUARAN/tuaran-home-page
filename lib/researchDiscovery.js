...
export function applyResearchDiscovery(xml, entries, kind) {
  ...
  const additions = entries.filter((entry) => entry.status === 'published' && !entry.encrypted).map((entry) => {
    ...
    if (kind === 'rss') return `<item><title>${escapeDiscoveryXml(entry.title)}</title><link>${url}</link><guid isPermaLink="true">${url}</guid><description>${escapeDiscoveryXml(entry.tldr || entry.summary || '')}</description>${validDate ? `<pubDate>${date.toUTCString()}</pubDate>` : ''}</item>`
