...
export async function buildResearchApprovalQueue(env, documents) {
  ...
  const pending = await fillMissingResearchTitles(classified.pending, async (item) => {
    const file = await fetchGitHubResearchFile(env, item.sourcePath)
    return file.raw
  })
  return { repo, files: titledFiles, pending, publishedCount: classified.publishedCount }
}
