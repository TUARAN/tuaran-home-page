...
export async function prepareResearchSourcePublication(db, { category, filename, raw }) {
  ...
  const snapshot = { entry, sourcePath: `research/${category}/${filename}`, sourceHash, status: 'published' }
  ...
  // publication is idempotent only for the exact same source and published state.
  ...
  return { snapshot, expectedRevision: current?.revision || 0, unchanged: current?.source_hash === sourceHash && current?.status === 'published' }
}
