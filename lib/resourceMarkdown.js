import { renderMarkdown } from './research/markdown.js'
...
export function prepareResourceLongformMarkdown(raw, { preludePattern } = {}) {
  ...
  lines = lines
    .filter((line) => !/^__TOC__\s*$/.test(line))
    .map((line) => {
...
export function extractResourceToc(markdown) {
  ...
  for (const line of lines) {
    ...
    if (volume) {
      ...
      const isPreface = /^(楔|前言|序言|说在前面)/.test(text)
      items.push({
        id: slugify(text) || `h-${items.length}`,
        text,
        depth: 2,
        kind: isVolume ? 'volume' : isPreface ? 'preface' : 'chapter',
      })
      continue
    }
    if (section) {
...
function tagVolumeHeadings(html, { treatAllH2AsChapters = false } = {}) {
  return html.replace(/<h2 id="([^"]+)">([\s\S]*?)<\/h2>/g, (full, id, inner) => {
    const plain = inner.replace(/<[^>]+>/g, '')
    if (!treatAllH2AsChapters && !/^第.+卷/.test(plain)) return full
    return `<h2 id="${id}" class="resource-volume-heading">${inner}</h2>`
  })
}

export function buildResourceLongformArticle(raw, { slug, title, preludePattern, treatAllH2AsChapters } = {}) {
...
export function buildCntWhitepaperArticle(raw) {
  return buildResourceLongformArticle(raw, {
    slug: 'cnt-whitepaper',
    title: 'CNT内容生态代币经济白皮书',
    preludePattern: /^#\s+一、项目摘要/,
    treatAllH2AsChapters: true,
  })
}

export function buildEthereumWhitepaperArticle(raw) {
  return buildResourceLongformArticle(raw, {
    slug: 'ethereum-whitepaper',
    title: '以太坊白皮书',
    preludePattern: /^#\s+新一代智能合约/,
    treatAllH2AsChapters: true,
  })
}

export function buildJianguoFanglyeArticle(raw) {
  return buildResourceLongformArticle(raw, {
    slug: 'jianguo-fanglye',
    title: '《建国方略》',
    preludePattern: /^#\s+第一卷/,
  })
}
