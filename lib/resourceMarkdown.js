...
export function prepareShenZhiDingNeiMarkdown(raw) {
  let lines = String(raw || '').split(/\r?\n/)

  const preludeIdx = lines.findIndex((line) => /^##\s+楔/.test(line))
...
export function buildShenZhiDingNeiArticle(raw) {
  const markdown = prepareShenZhiDingNeiMarkdown(raw)
  const toc = extractResourceToc(markdown)
  const html = tagVolumeHeadings(
    renderMarkdown(markdown, { seed: 'resource:shen-zhi-ding-nei', title: '置身钉内' }),
  )
