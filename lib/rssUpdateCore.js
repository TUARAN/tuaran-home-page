...
export function rssUpdateHref(feedId, guid = '') {
  const id = String(feedId || '').trim()
  if (!id) return RSS_UPDATE_PAGE_PATH
  const params = new URLSearchParams({ feed: id })
  const entryGuid = entryGuidValue(guid)
  if (entryGuid) params.set('entry', entryGuid)
  return `${RSS_UPDATE_PAGE_PATH}?${params.toString()}`
}

function entryGuidValue(guid) {
  return String(guid || '').trim().slice(0, 400)
}
