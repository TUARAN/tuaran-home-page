...
export function createSiteRegistry() {
  ...
  for (const site of sites.filter((item) => item.id !== 'main')) {
    ...
    if (ACCOUNT_SUBSITE_ORIGINS.includes(`https://${site.domain}`)) {
      relate(site.id, 'account', 'active', '通过主站子站会话接口读取身份；登录与退出均使用主站入口。')
    }
    // An allowed login origin alone does not mean points UI/check-in is integrated.
    if (site.id === 'weekly' || site.id === 'syncblog') {
