...
export const SECURITY_SELF_CHECK_SUMMARY = {
  ...
  conclusion: '策略元数据可按站长意愿公开；仓库公开前仍需处理第三方个人信息，并复核历史产物与加密记忆的离线破解风险。',
}
...
export const PUBLICATION_BLOCKERS = [
  ...
  },
  {
    level: '处理中',
    title: '加密记忆可被离线猜测口令',
    evidence: 'public/data/memory/ 跟踪了 AES-GCM 密文，清单同时公开 KDF 参数、密文和版本。',
    rationale: '算法和参数公开本身符合现代密码学原则，但任何人取得密文后都能无限离线尝试口令；风险取决于口令强度，而不是主题是否敏感。',
    action: '清除旧密文历史，轮换为随机高熵密钥，并拒绝用少于 24 字符的口令生成快照。',
  },
  {
    level: '需复核',
    title: 'Git 历史与 Actions 历史一起公开',
    evidence: '历史中存在记忆快照、.vercel/output 和 source map 等构建产物；现有敏感文件检查只审计文件路径和扩展名，不等同于完整内容与历史扫描。',
    rationale: '切换 Public 会一次性公开所有可达提交以及 Actions 日志；当前未发现明文凭据不等于能够证明历史中从未出现过凭据。',
    action: '使用专业秘密扫描器覆盖全部 refs，并逐项复核历史构建产物和工作流日志。',
  },
]
...
export const PUBLICATION_DECISION = [
  '如果站长明确接受个人策略、工程架构和运营方法公开，它们本身没有继续保持 Private 的充分理由。',
  '当前最合理的暂缓原因是第三方个人信息、公开密文带来的离线口令猜测面，以及尚未完成的全历史秘密扫描。',
  '完成三项处理后，可以重新评估将主仓库公开；若目的仅是获得免费 Actions，拆出最小公开自动化仓库仍然具有更小的长期暴露面。',
]
