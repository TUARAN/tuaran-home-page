...
export function getSeoGovernanceSnapshot() {
  const pages = Object.values(RICH_PAGE_SEO)
  const indexablePages = pages.filter((page) => page.robots?.index !== false)
  const metadataReady = pages.filter((page) => page.metadataTitle && page.description && page.canonical)
  const jsonLdReady = pages.filter((page) => page.schemaType && page.publishedTime && page.modifiedTime)
  const withImages = pages.filter((page) => page.image)
  const withKeywords = pages.filter((page) => page.keywords?.length)
  const canonicalCount = new Set(pages.map((page) => page.canonical)).size

  return {
    totals: {
      richPages: pages.length,
