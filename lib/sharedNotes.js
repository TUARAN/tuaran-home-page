...
import { SHARED_NOTE_MAX_CONTENT_LENGTH } from './shareContent'
...
export async function listSharedNotes() {
  ...
  try {
    const result = await db
      .prepare(
        `SELECT slug, title, content, envelope, created_at, updated_at, expires_at, view_count, last_viewed_at, created_by
         FROM shared_notes
    ...
    try {
      const result = await db
        .prepare(
          `SELECT slug, title, '' AS content, envelope, created_at, updated_at, expires_at, view_count, last_viewed_at, created_by
           FROM shared_notes
           ORDER BY created_at DESC
           LIMIT 200`
        )
...
/** 获取分享：默认只拿 envelope；owner 场景可显式带出 content。 */
export async function getSharedNote(slug, { includeContent = false } = {}) {
  if (!isValidSlug(slug)) return null
  const db = dbOrNull()
  if (!db) return null
  try {
    const columns = includeContent
      ? 'slug, title, content, envelope, created_at, updated_at, expires_at, view_count'
      : 'slug, title, envelope, created_at, updated_at, expires_at, view_count'
    const row = await db
      .prepare(
        `SELECT ${columns} FROM shared_notes WHERE slug = ?1`
      )
      .bind(slug)
      .first()
    if (!row) return null
...
export async function bumpViewCount(slug) {
  ...
  try {
    await db
      .prepare(
        'UPDATE shared_notes SET view_count = COALESCE(view_count, 0) + 1, last_viewed_at = ?2 WHERE slug = ?1'
      )
      .bind(slug, Date.now())
      .run()
  } catch {
...
export async function createSharedNote({ envelope, title, content, expiresInDays, createdBy }) {
  ...
  if (content.length > SHARED_NOTE_MAX_CONTENT_LENGTH) {
    return { ok: false, status: 413, error: 'CONTENT_TOO_LARGE', maxLength: SHARED_NOTE_MAX_CONTENT_LENGTH }
  }
  const db = dbOrNull()
