...
export async function listShortLinks(db, { q = '', limit = 20, offset = 0 } = {}) {
  ...
  const result = query
    ? await stmt.bind(like, safeLimit, safeOffset).all()
    : await stmt.bind(safeLimit, safeOffset).all()
  return result?.results || []
}

export async function countShortLinks(db, { q = '' } = {}) {
  const query = sanitizeText(q, 200)
  const like = `%${query}%`
  const sql = query
    ? `SELECT COUNT(*) AS total FROM short_links
       WHERE original LIKE ?1 OR short LIKE ?1 OR code LIKE ?1 OR title LIKE ?1 OR source LIKE ?1`
    : 'SELECT COUNT(*) AS total FROM short_links'
  const stmt = db.prepare(sql)
  const row = query ? await stmt.bind(like).first() : await stmt.first()
