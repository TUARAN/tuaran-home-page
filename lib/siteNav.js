/**
 * 站点导航分类（5 频道两级菜单 + /help 站点指南共享数据源）
 *
 * 一级频道：内容 / 工具 / 产品 / 圈子 / 关于（资源归入对应频道）
 * 内容频道按“入口 / 内容主题 / 内容类型”组织。
 * 每个 channel 含 sections（折叠分组）和扁平 routes 列表（站点帮助与后台使用）
 *
 * external: true 表示外链；tag 用于少量精选菜单角标
 * featured: true 表示频道总入口元数据，不代表当前页选中态
 * nav: false 表示不出现在主导航，但仍保留在 /help#site-map 全站导航
 * 注意：section 的中文 title 是 SiteHeader TIER_SECTION_STYLES 的样式键，改名要同步
 */

import { INTERACTIVE_DIRECTORY_WORKS } from './engineeringWorks'
...
function isInteractiveDirectoryPath(pathname) {
  const path = String(pathname || '').replace(/\/+$/, '') || '/'
  return INTERACTIVE_DIRECTORY_WORKS.some((work) => {
    const href = String(work.href || '').replace(/\/+$/, '')
...
export const SITE_CHANNELS = [
  {
    ...
    match: (p) =>
      ...
      p?.startsWith('/x-automation-retrospective') ||
      p?.startsWith('/web3') ||
      p?.startsWith('/a-share-research') ||
      p?.startsWith('/crypto-research') ||
      p?.startsWith('/adsense-content-check') ||
      p?.startsWith('/china-politics') ||
      p?.startsWith('/onchain-blog') ||
      p?.startsWith('/proofs') ||
      isInteractiveDirectoryPath(p),
    sections: [
      {
        title: '入口',
        titleEn: 'Start',
        items: [
          ...
          { href: '/onchain-blog', label: '链上作品', labelEn: 'Onchain Works', desc: '可直接使用并可核对的链上作品合集', descEn: 'Usable, verifiable works published onchain', nav: false },
          { href: '/onchain-blog/whitepaper', label: 'CNT 白皮书', labelEn: 'CNT Whitepaper', desc: '内容生态代币经济正式完整版', descEn: 'CNT content token economy whitepaper', nav: false },
          ...
          { href: '/web3', label: '市场与 Web3', labelEn: 'Markets & Web3', desc: 'A 股、加密与链上内容的统一入口', descEn: 'A-share, crypto and onchain content in one place', tag: 'New' },
          { href: '/web3/us-stocks', label: '美股走势', labelEn: 'US Stock Tape', desc: '苹果、SpaceX、特斯拉、Cloudflare、Twilio 的日 K 与名义占比', descEn: 'Daily candles and notional weights for five US stocks', nav: false },
          { href: '/a-share-research', label: 'A股调研', labelEn: 'A-Share Research', desc: '每天一家 A 股上市公司', descEn: 'One listed company at a time', tag: 'Auto', nav: false },
          { href: '/crypto-research', label: '加密调研', labelEn: 'Crypto Research', desc: '按市值观察加密资产', descEn: 'Crypto assets observed by market cap', tag: 'Auto', nav: false },
          { href: '/crypto-research/rss', label: 'RSS 订阅', labelEn: 'RSS Feeds', desc: '订阅本站，并阅读已收录源的更新', descEn: 'Subscribe here and read curated feed updates', nav: false },
        ],
      },
      {
        title: '内容主题',
        titleEn: 'By Topic',
        items: [
          { href: '/articles?subject=workbuddy', label: 'WorkBuddy', labelEn: 'WorkBuddy', desc: '产品、平台、教程与接入实践', descEn: 'Product, platform, guides and integrations', tag: 'New', nav: false },
          ...
          { href: '/articles?subject=web3', label: 'Web3 内容', labelEn: 'Web3 Content', desc: '公链、钱包、加密资产与链上应用', descEn: 'Chains, wallets, crypto and onchain apps', nav: false },
          ...
          { href: '/articles?subject=business_market', label: '商业与市场', labelEn: 'Business & Markets', desc: '行业、市场、增长与投资', descEn: 'Industries, markets, growth and investment', nav: false },
          ...
          { href: '/rich-pages', label: '互动专题', labelEn: 'Interactives', desc: '可阅读、可筛选、可操作', descEn: 'Readable and interactive content', nav: false },
          ...
          { href: '/resources/cz-memoirs', label: '币安人生', labelEn: 'CZ Memoir', desc: '赵长鹏自传简体中文全文', descEn: 'CZ memoir in simplified Chinese', nav: false },
          { href: '/resources/jianguo-fanglye', label: '建国方略', labelEn: 'Jianguo Fanglue', desc: '孙文学说、实业计划、民权初步全文', descEn: 'Sun Yat-sen’s three reconstruction books', nav: false },
          { href: '/onchain-blog/whitepaper', label: 'CNT 白皮书', labelEn: 'CNT Whitepaper', desc: '内容生态代币经济正式完整版', descEn: 'CNT content token economy whitepaper', nav: false },
          { href: '/resources/ethereum-whitepaper', label: '以太坊白皮书', labelEn: 'Ethereum Whitepaper', desc: 'Vitalik Buterin 2014 年白皮书中文全文', descEn: '2014 Ethereum white paper in Chinese', nav: false },
      ...
      },
      {
        ...
        titleEn: 'Type',
        items: [
          { href: '/rich-pages', label: '互动', labelEn: 'Interactives', desc: '可阅读、可筛选、可操作', descEn: 'Readable, filterable and interactive', tag: '新体验' },
          { href: '/articles?group=resource', label: '资源', labelEn: 'Resources', desc: '档案、下载、订阅与收藏', descEn: 'Archives, downloads and subscriptions' },
          { href: '/articles?group=article', label: '精选', labelEn: 'Featured', desc: '观点、记录与长文', descEn: 'Essays, notes and long-form writing' },
          { href: '/articles?group=analysis', label: '分析', labelEn: 'Analysis', desc: '深度分析、核验与对象档案', descEn: 'Analysis, verification and profiles', nav: false },
          { href: '/articles?group=practice', label: '实践', labelEn: 'Practice', desc: '工程案例、实作记录与指南', descEn: 'Cases, build logs and guides', nav: false },
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        titleEn: 'Tools',
        items: [
          { href: '/tools', label: '工具集', labelEn: 'Tool Collection', desc: '在线工具、扩展、应用与开发实验', descEn: 'Online tools, extensions, apps & experiments', featured: true },
          { href: '/downloads', label: '下载中心', labelEn: 'Download Center', desc: '扩展与客户端', descEn: 'Extensions and apps' },
          { href: '/tools/workbuddy-desktop-pet', label: '鹿鹿精灵', labelEn: 'Lulu Desktop Pet', desc: '长颈鹿桌宠与网页互动演示', descEn: 'Desktop pet and interactive web preview', nav: false },
      ...
      },
      {
        ...
        items: [
          ...
          { href: '/codex-reset', label: 'Codex 重置', labelEn: 'Codex Reset', desc: '额度重置与发卡日历', descEn: 'Quota reset and credit calendar', tag: 'New' },
      ...
      },
      {
        ...
        items: [
          ...
          { href: '/tools/code-miner', label: '代码矿工', labelEn: 'Code Miner', desc: '开发者工具集合', descEn: 'Developer tool collection' },
      ...
      },
      {
        ...
        items: INTERACTIVE_DIRECTORY_WORKS.map((work) => ({
          href: work.href,
          label: work.title,
  ...
  },
  {
    ...
    match: (p) =>
      ...
      p?.startsWith('/workbuddy-publish-center') ||
      p?.startsWith('/context-memory'),
    sections: [
      {
        title: '产品与能力',
        titleEn: 'Products & Capabilities',
        items: [
          { href: '/works', label: '产品集', labelEn: 'Portfolio', desc: '独立产品、站内工具与工程作品全览', descEn: 'Products, built-in tools & engineering works', featured: true },
          { href: '/capabilities', label: 'Agent 能力集', labelEn: 'Agent Capabilities', desc: 'Skill、MCP、Prompt 与 WorkBuddy 能力包', descEn: 'Skills, MCP services, prompts and WorkBuddy packages' },
    ...
    sections: [
      {
        ...
        titleEn: 'Site',
        items: [
          { href: '/help', label: '站点帮助', labelEn: 'Site Help', desc: '关于、导航、使用、规则与联系', descEn: 'About, navigation, usage, policies & contact', featured: true },
          { href: '/changelog', label: '更新记录', labelEn: 'Changelog', desc: '按周、月、季、年查看本站演进', descEn: 'Site evolution by week, month, quarter, or year' },
          ...
          { href: '/about/resume', label: '求职简历', labelEn: 'Resume', desc: '一页式履历，可打印', descEn: 'One-page printable résumé', nav: false },
...
/** 从可见菜单中选出最具体的当前入口；筛选链接优先于同路径总入口。 */
export function getActiveNavHref(sections, pathname, searchParams = new URLSearchParams()) {
  if (!pathname) return null
  const currentPath = pathname.replace(/\/+$/, '') || '/'
  // 旧能力页保留为详情入口，菜单选中态统一归到 Agent 能力集。
  if (sections.some((section) => section.items.some((item) => item.href === '/capabilities')) &&
    ['/skill-center', '/mcp-center', '/prompt-center', '/workbuddy-publish-center'].some((path) =>
      currentPath === path || currentPath.startsWith(`${path}/`))) return '/capabilities'
  let activeHref = null
  let bestPathLength = -1
  let bestQueryCount = -1
  for (const item of sections.flatMap((section) => section.items)) {
    if (item.external || !item.href?.startsWith('/') || item.href.startsWith('//')) continue
    const url = new URL(item.href, 'https://2aran.com')
    if (url.hash) continue
    const path = url.pathname.replace(/\/+$/, '') || '/'
    if (currentPath !== path && !(path !== '/' && currentPath.startsWith(`${path}/`))) continue
    const query = [...url.searchParams.entries()]
    if (query.length && (currentPath !== path || !query.every(([key, value]) => searchParams.getAll(key).includes(value)))) continue
    if (path.length > bestPathLength || (path.length === bestPathLength && query.length > bestQueryCount)) {
      activeHref = item.href
      bestPathLength = path.length
      bestQueryCount = query.length
    }
  }
  if (!activeHref) {
    const mapsToRichPages = sections.some((section) =>
      section.items.some((item) => item.href === '/rich-pages'))
    if (mapsToRichPages && INTERACTIVE_DIRECTORY_WORKS.some((work) => {
      const href = String(work.href || '').replace(/\/+$/, '')
...
export const SITE_FOOTER_LINKS = [
  { href: '/help', label: '站点帮助', labelEn: 'Site Help', desc: '关于 / 导航 / 使用', descEn: 'About / navigation / usage' },
  { href: '/about', label: '关于站长', labelEn: 'About the Owner', desc: '站长介绍 / 履历', descEn: 'Owner bio & résumé' },
  { href: '/help#editorial', label: '内容说明', labelEn: 'Editorial Policy', desc: '作者责任 / 工具 / 更正', descEn: 'Authorship / tools / corrections' },
  { href: '/help#privacy', label: '隐私政策', labelEn: 'Privacy', desc: '数据 / Cookie / 广告', descEn: 'Data / cookies / ads' },
  { href: '/help#contact', label: '联系', labelEn: 'Contact', desc: '邮件 / 微信 / 合作', descEn: 'Email / WeChat / collaboration' },
...
/** 扁平展开所有路由，供站点指南与后台使用；传入 account 时按身份过滤。 */
...
/**
 * 主导航只展示精选入口；/help#site-map 展示完整公开入口。
...
/** 完整导航需要保留 nav:false 项目，但仍按 account 过滤可见性。 */
