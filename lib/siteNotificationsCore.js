...
const INTERACTION_NOTIFICATION_TYPES = new Set(['comment_reply', 'content_comment', 'content_like'])

export function isInteractionNotification(type) {
  return INTERACTION_NOTIFICATION_TYPES.has(String(type || ''))
}

/** 告警摘要：任务名 + 状态 + 错误摘要 + 运行链接，控制在 160 字内。 */
export function buildAutomationAlertExcerpt({ taskName = '', status = 'failed', error = '', runUrl = '' } = {}) {
  const ok = ['ok', 'success'].includes(String(status || '').toLowerCase())
  const head = `${String(taskName || '自动化任务').slice(0, 30)} 运行${ok ? '成功' : '失败'}`
