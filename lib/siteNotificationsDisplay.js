...
export const NOTIFICATION_TYPE_META = {
  ...
  weekly_summary: {
    ...
    typeLabel: '周报',
    actionLabel: '打开周报',
    href: '/admin/content-weekly?days=7',
  },
  automation_monitor: {
    category: 'automation',
    typeLabel: '监控',
...
export function notificationHref({ type, articleKey, commentId, contentHref } = {}) {
  ...
  if (type === 'rss_update') {
    const feedId = parseRssArticleKey(articleKey).feedId
    const href = rssUpdateHref(feedId)
    return feedId ? `${href}#rss-feed-${feedId}` : href
  }

  const pageHref = String(contentHref || '').trim()
  if (!pageHref) return meta.fallbackHref || '/notifications'

  if (type === 'content_like') return pageHref
