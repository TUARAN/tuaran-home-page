import { SPACEX_ARCHIVED_LAUNCHES } from './spacexArchivedLaunches.js'

export { SPACEX_ARCHIVED_LAUNCHES } from './spacexArchivedLaunches.js'
...
export const SPACEX_EDITORIAL_ENTRIES = [
  ...
  },
  {
    ...
    video: {
      src: '/videos/starship-flight-5-first-booster-catch-2024-10-13.mp4',
      label: 'Flight 5 首次用发射塔机械臂捕获 Super Heavy（塔台视角）',
      credit: '影像：SpaceX',
      postUrl: 'https://x.com/SpaceX/status/1845922924315938922',
    },
  },
  {
  ...
  {
    id: 'spacex-holy-grail-of-rocketry-2026',
    publishedAt: '2026-09-16T21:23:31Z',
    kind: 'spacex',
    topic: 'Starship',
    title: '《The Holy Grail of Rocketry》：完全复用目标的工程路径',
    titleEn: 'The Holy Grail of Rocketry: the engineering path to full reusability',
    summary: 'SpaceX 发布 Starship 系列影片，回顾团队从早期火箭回收实验走向完全、快速复用，并以 Flight 13 和 Ship 40 回收工作展示 Starship 的最新验证进度。',
    summaryEn: 'SpaceX continues its Starship film series with a look back at the engineering work behind rocket recovery and the path toward full, rapid reusability, including Flight 13 and the recovery effort for Ship 40.',
    sourceLabel: 'SpaceX · The Holy Grail of Rocketry',
    sourceUrl: 'https://x.com/SpaceX/status/2100334792646549729',
    note: 'SpaceX 官方影片；本地归档为 90 秒竖版片段',
    noteEn: 'Official SpaceX film; the local archive is a 90-second vertical excerpt',
    video: {
      src: '/videos/starship-holy-grail-of-rocketry-2026-09-16.mp4',
      label: 'Starship Flight 13 与完全快速复用目标的官方影片片段',
      credit: '影像：SpaceX',
      postUrl: 'https://x.com/SpaceX/status/2100334792646549729',
    },
  },
].map((entry) => ({
  ...entry,
...
async function fetchLaunchPage(fetchImpl, phase) {
  ...
  try {
    ...
    return {
      entries,
      total: Number.isFinite(Number(payload?.count)) ? Number(payload.count) : entries.length,
    }
  } finally {
...
export async function getSpacexTimeline(fetchImpl = fetch) {
  ...
  const launches = results.flatMap((result) => (result.status === 'fulfilled' ? result.value.entries : []))
  ...
  return {
    entries,
    stats: {
      historicalLaunchCount: results[1].status === 'fulfilled' ? results[1].value.total : null,
      archivedLaunchCount: SPACEX_ARCHIVED_LAUNCHES.length,
      archivedVideoCount: [...SPACEX_ARCHIVED_LAUNCHES, ...SPACEX_EDITORIAL_ENTRIES]
        .reduce((count, entry) => count + Number(Boolean(entry.video)) + (entry.videos?.length || 0), 0),
      recentLaunchCount: uniqueLaunches.filter((entry) => entry.phase === 'previous').length,
      upcomingLaunchCount: uniqueLaunches.filter((entry) => entry.phase === 'upcoming').length,
    },
