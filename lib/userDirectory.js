...
/**
 * 用户目录（site_users 表，迁移 0021；账号模块迁移 0047）
 *
 * 数据流：
    ...
    await db
      .prepare(
        `INSERT INTO site_users
           (id, platform_id, provider, login, name, email, image, role, note, first_seen_at, last_seen_at, login_count)
         VALUES (?1, ?1, ?2, ?3, ?4, ?5, ?6, 'member', '', ?7, ?7, 1)
...
export async function getUserRole(userId) {
  ...
  try {
    const row = await db
      .prepare('SELECT role FROM site_users WHERE platform_id = ?1 OR id = ?1 LIMIT 1')
      .bind(id)
      .first()
...
export async function listSiteUsers(db) {
  ...
  return (result?.results || []).map((row) => ({
    ...
    platformId: row.platform_id || row.id,
...
export async function updateSiteUser(db, id, { role, note }) {
  ...
  binds.push(id)
  const result = await db
    .prepare(`UPDATE site_users SET ${sets.join(', ')} WHERE platform_id = ? OR id = ?`)
    .bind(...binds, id)
    .run()
