import { X_MEME_SLOT_THEMES, X_MEME_VERSION, pickXMemeAsset } from './xMemeAssets.js'
...
export async function prepareXImage(options) {
  ...
  if (options.memeOrigin && X_MEME_SLOT_THEMES[asset.row.slot]) return prepareXMemeImage(options)
...
export async function prepareXMemeImage({ db, bucket, asset, memeOrigin, fetchImpl = fetch }) {
  // A saved image belongs to its draft, even after the catalog changes.
  if (asset.row.object_key?.startsWith(`${X_IMAGE_PREFIX}memes/`)) {
    if (!bucket) throw assetError('X_IMAGE_R2_NOT_CONFIGURED', 503)
    const saved = await bucket.get(asset.row.object_key)
    if (!saved) throw assetError('X_IMAGE_STORED_OBJECT_MISSING')
    await updateXAsset(db, asset, { status: 'ready', error: '' })
    return new Blob([await saved.arrayBuffer()], { type: asset.row.mime_type })
  }
  const meme = pickXMemeAsset({ slot: asset.row.slot, date: asset.row.date_key })
  if (!meme) throw assetError('X_MEME_SLOT_UNAVAILABLE')
  if (!bucket) throw assetError('X_IMAGE_R2_NOT_CONFIGURED', 503)
  const objectKey = `${X_IMAGE_PREFIX}memes/${X_MEME_VERSION}/${meme.id}.png`
  let stored = await bucket.get(objectKey)
  let bytes
  if (stored) {
    bytes = await stored.arrayBuffer()
  } else {
    const response = await fetchImpl(new URL(meme.path, memeOrigin), { signal: AbortSignal.timeout(20_000) })
    if (!response.ok) throw assetError('X_MEME_FETCH_FAILED')
    bytes = await response.arrayBuffer()
    const signature = new Uint8Array(bytes).slice(0, 8)
    if (bytes.byteLength > 5 * 1024 * 1024 || signature.join(',') !== '137,80,78,71,13,10,26,10') {
      throw assetError('X_MEME_INVALID_PNG')
    }
    await bucket.put(objectKey, bytes, { httpMetadata: { contentType: 'image/png' } })
  }
  await updateXAsset(db, asset, {
    object_key: objectKey, mime_type: 'image/png', size_bytes: bytes.byteLength,
    image_model: '原创表情包', prompt: meme.label, asset_source: 'pool', pool_asset_id: '',
...
export function xPoolView(row) {
  return { id: row.id, label: row.title, contentType: row.content_type, objectKey: row.object_key,
    ...
    thumbUrl: row.thumbnail_object_key ? `/api/admin/morning-greeting/assets/${encodeURIComponent(row.id)}?pool=1&thumb=1` : '',
