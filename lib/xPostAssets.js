import { X_MEME_ASSETS, X_MEME_VERSION } from './xMemeAssets.js'
...
export async function prepareXImage(options) {
  ...
  if (options.memeOrigin && X_MEME_ASSETS[asset.row.slot]) return prepareXMemeImage(options)
...
export async function prepareXMemeImage({ db, bucket, asset, memeOrigin, fetchImpl = fetch }) {
  const meme = X_MEME_ASSETS[asset.row.slot]
  if (!meme) throw assetError('X_MEME_SLOT_UNAVAILABLE')
  if (!bucket) throw assetError('X_IMAGE_R2_NOT_CONFIGURED', 503)
  const objectKey = `${X_IMAGE_PREFIX}memes/${X_MEME_VERSION}/${asset.row.slot}.png`
  ...
  await updateXAsset(db, asset, {
    ...
    image_model: '原创猫咪表情包', prompt: meme.label, asset_source: 'pool', pool_asset_id: '',
