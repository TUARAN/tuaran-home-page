-- 草稿状态补充 'published'（站长复核后已发布到 research/companies 并推送）。
-- 0060/0062 的 CHECK 没有该状态，且需要记录发布 commit，重建表并保留现有数据。
CREATE TABLE a_share_drafts_new (
  ...
  status TEXT NOT NULL DEFAULT 'pending'
    CHECK (status IN ('generating', 'pending', 'reviewed', 'rejected', 'published')),
...
INSERT INTO a_share_drafts_new
  (id, code, name, title, draft_date, content, template_version, style_id,
   deepseek_task_id, attempt_count, status, publish_commit, publish_at, created_at, updated_at)
SELECT
  id, code, name, title, draft_date, content, template_version, style_id,
  deepseek_task_id, attempt_count, status, '', NULL, created_at, updated_at
FROM a_share_drafts;
