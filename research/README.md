...
发布流程：把 Markdown 推到 `main`（纯内容提交用 `[CF-Pages-Skip]`），打开站长后台 `/admin/articles/research-import`，在「审批调研」里核对正文后点发布。修改或撤回也从该入口进行。须先应用 `migrations/0089_content_documents.sql` 并部署新版读取代码；Git 提交只保存创作正本，不会自动写线上数据库。
...
| `updated` | ⭕ | 最后一次实质修改的 `YYYY-MM-DD`，或带明确时区的 ISO 时间，如 `2026-09-09T14:30:00+08:00`；不填写则修改时间回退到发布日期 |
| `tags` | ⭕ | 字符串数组，用于检索与归档 |
| `summary` | ⭕ | 一句话摘要，列表页展示 |
...
- `date` 是首次发布日期，更新正文时保持不变；`time` 是可选的首次发布时间，按北京时间解释。
- `updated` 只在事实、结论、步骤或资料发生实质更新时填写。构建、部署、格式微调不自动修改它。
- 日期须为真实日历日期；精确的 `updated` 使用 `YYYY-MM-DDTHH:MM:SS[.SSS]Z` 或带 `±HH:MM` 偏移的同格式，拒绝没有时区的时间。不要另填隐含本地时区的字符串。
- 为兼容历史输出，仅日期值仍以 UTC 零点转换为机器时间；有 `time` 的发布日期使用 `+08:00`。只有日期时不声称知道实际发生时刻；同日需要精确区分先后时填写带时区的 `updated`。
- 缺失或非法 `date` 回退到文件名中的合法日期；两者都无效时不输出发布时间。非法 `time` 被忽略，仍保留合法日期。
- 非法或早于发布时间的 `updated` 被忽略，修改时间回退到有效发布时间；两者都无效时省略修改时间及 sitemap `lastmod`。若仅 `updated` 有效，可输出修改时间，发布时间仍省略。
- loader 统一输出 `publishedTime`、`modifiedTime`（UTC ISO），详情 Open Graph、JSON-LD 与 sitemap 使用同一字段；`updated` 保留通过校验的原始值。列表继续按首次发布时间排序。
- 不使用当前时间、部署时间或文件 mtime 补齐内容日期，重复构建应得到相同结果。
...
2. 关键事实有一手来源；估算、推断和会改变结论的持续验证项已明确标注；
...
- 事实与研判必须分开。会改变结论的缺项才写入文末「持续验证」，不要补成结论，也不要单独做成大节；不会改变结论的缺口可以不写。
...
骨架是帮手，不是要填满的表格。决定判断的点写透，背景和缺项收短。

```markdown
## 一、先给结论
一句话结论 + 3–5 个已经能站住的要点。不要用缺项开场。

## 二、事实层
公开资料、时间线、基本信息。按重要性写，不必面面俱到。

## 三、结构分析
商业模式、技术链路、组织约束、利益相关方。写透真正卡住判断的那一层。
...
需要标明边界的说明，写成 frontmatter 之后、第一个标题之前的一条引用。渲染器会收成一行摘要并挪到文末，点开再看全文。

```markdown
> 适用范围：讨论的是健康成人在公开、被评价、结果不完全可控时的急性应激与表达表现。……不构成医疗建议。

> **风险与合规提示：** 资料用于理解产品机制，不构成投资、开户或法律建议。……
```

白名单标签（必须出现在引用开头，后接全角或半角冒号）：

- `适用范围`
- `风险与合规提示`
- `风险提示`
- `合规提示`（括号补充可保留，如 `合规提示（需自行核实最新法规）`）

用法：

- 只在省略后会误导、或涉及医疗、法律、投资、安全时使用。不要给每篇文章套一段。
- 用引用，不用二级标题，也不要写成「写在前面」「说在前面」「免责声明」「资料口径」。
- 一条引用说完；标签后直接写边界，不要再套一层「本文 / 本调研」。
- 稿件仍写在文首引用里；渲染器把这些说明统一挪到正文之后。
- 正文里出现「缩小适用范围」或普通引语，不会折叠。
- 新增标签必须同时改 `lib/research/markdown.js` 的 `COLLAPSIBLE_NOTE_LABEL_RE`、对应测试和本段。不要临时发明标签。

医疗、法律、投资等必要风险提示，可能影响读者安全的说明，重大更正，以及会直接影响真实性的 OCR 错误或原文残缺提示，继续用上面的白名单。骨架可随风格和题材调整；资料档案、投研备忘等请以风格库对应配置为准。
...
上述跳过构建前缀仅用于纯内容提交；代码变更仍按正常流程部署。Git 提交用于保存创作正本。新版部署后，打开 `/admin/articles/research-import` 审批发布，无需等待 Pages 重建，也不用再导出 JSON；已部署的 A 股/加密资产专用发布入口会同步写入 D1。发布成功后访问：
