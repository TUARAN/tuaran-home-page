...
| `source` | ⭕ | 协助工具：`claude-code` / `cursor` / `codex` / `doubao` / `gemini` / `gpt` / `manual`；前台按模型/工具短标签展示，例如 `claude-code` 显示 `来源：Opus 4.7`，`cursor` 显示 `来源：Composer 2.5`，`codex` 显示 `来源：GPT5.5` |
