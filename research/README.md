...
| `date` | ✅ | `YYYY-MM-DD` |
| `time` | ⭕ | `HH:MM` 或 `HH:MM:SS`（北京时间）；**新调研建议填写**。有则列表与 RSS 按日期+时间排序；旧文章无此字段仍只显示日期 |
| `tags` | ⭕ | 字符串数组，用于检索与归档 |
| `summary` | ⭕ | 一句话摘要，列表页展示 |
| `tldr` | ⭕ | 详情页顶部 TL;DR 框使用；不写则回退 `summary` 或正文首段 |
| `topic_type` | ⭕ | 仅 `topics` 用，二级类型：`industry` 行业 / `tech` 技术 / `product` 产品 / `market` 市场 / `thesis` 观点 |
| `assistance` | ⭕ | 协助工具：`claude-code` / `cursor` / `codex` / `doubao` / `gemini` / `gpt` / `manual`；前台按模型/工具短标签展示，例如 `claude-code` 显示 `协助：Opus 4.7`，`cursor` 显示 `协助：Composer 2.5`，`codex` 显示 `协助：GPT5.5`。旧文章的 `source` 字段仍兼容读取，但新文章建议使用 `assistance` |
...
- 未指定时使用「默认调研风格」；人味、周刊解释、投研备忘、资料档案等风格按主题选用。
- 事实与研判必须分开；没有公开证据的内容写入「未能验证」，不要补成结论。
- 每个关键数字、日期、版本、价格都有来源，或明确标为估算。
- 所有调研共同遵守通用禁语。`npm run research:style-audit` 会扫描候选句；它是提示器，不会替代人判断「不是 X，而是 Y」等必要对比。
