...
tldr: 这轮面试是在验证你是否真的做过 Agent 工程：Memory 怎么抽取、检索、更新，BM25 和向量检索怎么融合，大规模记忆库怎么提效，ReAct 与 Workflow 如何取舍，以及如何用评测和后训练让 Agent 持续变好。
assistance: codex
model: gpt-5
pv: 0
---

把图片中流传的「千问 C 端 Agent 算法日常实习一面」题单整理成可复用的面试答案。题目表面上问 Agent、Memory、RAG、ReAct、后训练，实际重点是：候选人能不能把论文 / 项目里的方法落成一个可评测、可迭代、可上线的 C 端 Agent 系统。
