...
summary: 「模型大到一定程度就会涌现」是 2022 年的经典叙事；2023 年被「海市蜃楼」论文打回原形，2024-2026 年又被小模型 + 后训练 + 推理 RL 彻底改写。梳理这条结论的兴衰史，并给出今天还能不能用「参数量阈值」来谈涌现的判断。
...
这篇调研的目的是把这条结论的兴衰路径讲清楚，让今天再被问到「多少 B 会涌现」时知道该如何回答。
...
如果这条结论站不住，上面这些判断的根基就被抽走了。所以它是关系到投资方向、研发路线、安全叙事的核心命题。
...
2. 后续如果要写「为什么 7B 模型已经够用」「推理时算力 vs 训练时算力的成本经济学」这类延伸主题，可以反向引用这项分析作为背景。
3. 在与非技术朋友讨论大模型时，遇到「多少参数才算大模型」这类提问，用上述结论快速纠偏：**问错了问题**。
