...
要做对这件事，先理解 Anthropic 官方对 workflow 和 agent 的明确区分；然后看一线工程师（Erik Schluntz）反复强调的"最小框架"主张；最后看 2026-04 三-agent harness、Claude Managed Agents 这类"完整 harness 形态"的事实，把它们放在一条升级路径上对照。多数人需要的是知道自己应该停在哪一阶段。
