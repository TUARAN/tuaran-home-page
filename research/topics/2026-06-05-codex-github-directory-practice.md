...
tldr: 项目目录失控时，真正的问题通常是缺少状态、体积、归属和风险台账；Codex 适合把本地文件系统、Git 状态和工程判断串成一次可审计的整理流程。
topic_type: tech
tech_type: ai_coding
...
所以这次是一次工程资产盘点：先知道有什么，再判断为什么大，再决定哪些能清、哪些该保留、哪些先归档。
...
因此，清理策略是只清理明确可再生的目录：`node_modules`、`.next`、`target`、`dist`、`build`、profile、备份目录。源码改动一律不碰。
...
这个动作是降级。它的意义是让 `/Users/tuaran/Documents/github` 主目录重新变成“Git 仓库列表 + `_archive`”，减少每天打开目录时的噪音。
...
这次整理释放了大约 `38G` 空间，但真正的收益是把本地工程资产重新分层：
