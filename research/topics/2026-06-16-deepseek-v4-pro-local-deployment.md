...
date: 2026-06-16
time: 16:40
tags: [大模型, 本地部署, DeepSeek, DeepSeek-V4, MoE, 量化, 显存, vLLM, llama.cpp, Ollama, 推理, 私有化, GPU, 成本预算, API定价, 盈亏平衡]
...
你想"在本地跑 DeepSeek-V4"，能装进单机/消费级显卡的是 **V4-Flash（284B/激活13B）**；**V4-Pro（1.6T/激活49B）是集群机型，单卡双卡乃至 8×H100 单机都装不下**——给 Pro 谈"4090 单卡 token/s"是伪命题。
...
## 九、成本预算（自购 / 云租 / API 三条路）

本节核心矛盾：**很多人盯着"买几张卡"，但 V4 这代 API 已永久降价到极低，自建在多数场景是更贵的那条路——成本预算要先把 API 当默认基线。**

> 价格口径：① **API 为官方定价**（已确认，2026-05-22 永久降价后）；② **GPU 售价 / 云租为市场参考区间**（intuitionlabs、cloudzero、spheron、thundercompute 等 2026-06 整理，**非官方、随行就市**）；③ 汇率按 **~¥7.1/$** 粗估，仅供量级参考。

### 9.1 三条路线总览

| 路线 | 一句话 | 适合 |
|---|---|---|
| **API 直连** | 不买卡、按 token 付费、官方精度 | 绝大多数团队的默认起点 |
| **云租 GPU 自建** | 按小时租卡跑 Flash/Pro | 阶段性高负载、要数据可控但不想买硬件 |
| **硬件自购** | 一次性买卡长期持有 | 数据不出域、已有机房、长期满负荷 |

### 9.2 硬件自购：单卡市场参考价（2026-06）

| 显卡 | 显存 | 参考单价（USD） | 约合人民币 |
|---|---|---|---|
| RTX 4090 | 24 GB | $1,600–2,000 | ¥1.2–1.5 万 |
| RTX 6000 Ada | 48 GB | $6,800–7,500 | ¥5–5.5 万 |
| A100 80GB | 80 GB | $15,000–18,000 | ¥11–13 万 |
| H100 80GB | 80 GB | $27,000–40,000 | ¥20–29 万 |
| H200 141GB | 141 GB | ~$39,000（8 卡整机 ~$315,000） | ¥28 万 / 整机 ¥224 万 |

整机预算（含主机/电源/散热的**估算**，非官方报价）：

| 档位 | 配置 | 卡成本 | 整机估算 |
|---|---|---|---|
| 个人 Flash Q4 | 4×RTX 4090 | ~$7K | **~$10–13K（¥7–9 万）** |
| 私有服务 Flash FP8 | 4×A100 80GB | ~$60–72K | **~$75–95K** |
| 私有服务 Flash 原生 | 2×H200 | ~$78K | **~$90–110K** |
| Pro 集群（单节点起） | 8×H200 | ~$315K | **$350K+/节点，Pro 还需多节点** |

### 9.3 云租：按小时市场区间（2026-06，spot ~ on-demand）

| 资源 | 每小时区间 | 整月持有估算（720h） | 备注 |
|---|---|---|---|
| RTX 4090 | $0.20–0.40 | ~$150–290 | 多卡才装得下 Flash |
| A100 80GB | $0.78–1.99 | ~$560–1,430 | — |
| H100 80GB | $1.03–3.29（AWS ~$6.88） | ~$740–2,370 | spot 最低 |
| H200 141GB | ~$0.50–3 | ~$360–2,160 | 低价区波动大 |
| 8×H100（Flash 服务） | $33（1yr 预留）–55（按需） | ~$2.4 万–4 万 | p5 级整机 |
| 8×H200（Pro/大并发） | ~$24（1yr 预留）起 | ~$1.7 万起 | 跑 Pro 仍需多节点 |

### 9.4 API 直连：官方定价（已确认，永久降价后）
...
> 直观换算：Flash 处理 **100 万字中文问答（约 150 万 token 进 + 50 万出）** ≈ 输入 $0.21 + 输出 $0.14 ≈ **$0.35（¥2.5 元）**。缓存命中时输入再降两个数量级。

### 9.5 盈亏平衡测算（外部测算，非官方）

> 下列为按公开价格做的**测算区间**，不是承诺数字；自建实际成本受利用率、电费、运维人力影响极大。

| 对照 | API（Flash 输出） | 自建 8×H200 跑 Flash（折合输出成本） |
|---|---|---|
| 单位成本 | **$0.28 / M** | ~$1.20/M（90% 利用）~ $2.50/M（60% 利用） |
| 结论 | — | **多数利用率下自建仍贵于 API** |

经验法则（来自第三方测算，2026）：

- **V4-Pro 月推理支出 < ~$15K，别自建**，留在 API 更省。
- Flash 用 8×H100 云租自建，**约 3–4 B token/天**才追平 API——这是很高的日吞吐门槛。
- **自建只有四种情况真划算**：① 数据不出域（合规硬要求）；② 需要 <100ms 稳定低延迟；③ 已有闲置 GPU 要摊销；④ 必须做定制微调。除此之外，**API 几乎总是更便宜的那条路**。

---

## 十、研判：这套硬件账怎么算才不踩坑（观察）
...
2. **V4 这代真正的解锁点是 KV cache 省到 ~2%。** 它是"长上下文的边际显存成本被压下来"，所以 1M 才落地。本地部署最该盯的指标因此从"参数量"转向"KV/上下文预算"。
...
5. **成本上，API 是默认基线，自建是例外。** V4 这代官方定价已压到极低（Flash 输出 $0.28/M），自建即便满负荷也很难做到更便宜；先用 API 跑，月支出过万美元或合规要求倒逼时再评估自建——把顺序倒过来就是在替闲置 GPU 找活干。
...
- 第九节 GPU 售价 / 云租：为 2026-06 市场参考区间，随行就市波动，**非官方报价**；唯 API 定价为官方确认。盈亏平衡为外部测算，未含电费/运维人力。
...
- DeepSeek 官方只公开架构参数与基准分；逐显卡 token/s、GGUF 各量化档显存和并发上限未发布。正文的这三类数字来自第三方社区整理或“激活参数 × 显存带宽”的工程估算，并逐表标注。
- 作者未拿到 V4-Pro 集群实测；本文是外部整理与测算，不是官方白皮书或实验室报告。未公开数据集中列入“未能验证”。资料截至 2026-06-16。
...
- 成本 / 价格（市场参考，非官方）：intuitionlabs「NVIDIA AI GPU Pricing」、cloudzero「H100 GPU Cost / DeepSeek Pricing」、spheron / thundercompute / synpixcloud GPU 云租对比、aitechconnect「V4-Pro 8×H100 break-even」、codersera「V4-Pro 永久降价」（API 定价为官方口径）。
