...
这篇应该归在「行业调研」，更准确说是 `topics` 里的 `industry`。

原因很简单：这不是一家公司调研，也不是某个人物调研。它讨论的是一个行业正在发生的范式变化。

2023 年前后，大家对 AI 的主要感受还是「它会聊」。到了 2025 到 2026 年，核心问题变成了「它能不能替我做」。这一步看起来只是多了几个工具，实际差很多。
...
| 条件 | 具体是什么 |
|---|---|
| 工具 | 浏览器、代码解释器、API、文件系统、企业软件 |
| 执行环境 | 云端电脑、本地终端、沙箱、浏览器会话 |
| 任务管理 | 能拆步骤、记录进度、失败后继续 |
| 权限边界 | 哪些动作要确认，哪些数据不能碰 |
| 人类验收 | 最后还是人判断它有没有真的完成 |

所以我不太想把这件事写成普通产品榜单。榜单会很快过期。更值得看的是：谁已经能接进真实工作流，谁还主要停在对话、演示和轻任务里。

## 三、我目前看到的几条线

### 3.1 Codex：目前最普世好用的工作型 agent
...
### 3.4 Project Astra / Gemini：更像未来的随身 AI
...
### 3.5 Microsoft Copilot Studio：企业不会先要一个万能机器人
...
### 3.6 OpenClaw / Manus / Genspark / Comet：高手的工作台
