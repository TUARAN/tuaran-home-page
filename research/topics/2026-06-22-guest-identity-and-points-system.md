---
title: 游客身份与燃币体系分析：匿名评论、ID 绑定与燃币门槛的通用方案 + 2aran.com 落地设计
category: topics
date: 2026-06-22
time: 17:40
tags: [游客身份, 匿名评论, 设备指纹, 签名Cookie, 账号绑定, 燃币体系, 内容门槛, Cloudflare D1, edge, 反作弊]
summary: 结合公开方案与 2aran.com 现有架构，分析「免登录游客也能评论 + 后续注册可绑定」和「注册得燃币、燃币解锁资源」两类需求；对比 Cookie UUID / 签名 Cookie / IP / 设备指纹的稳定性、隐私与防滥用取舍，并给出燃币账本（ledger）、D1 表结构与分期建议。
...
# 游客身份与燃币体系分析：匿名评论、ID 绑定与燃币门槛的通用方案 + 2aran.com 落地设计
...
**游客身份用「签名 Cookie 里的随机 UUID」当主键，IP 和设备指纹只当反滥用的弱信号；燃币用「只增不改的流水账本」当正本，余额只是它的物化缓存。**
...
| 燃币直接改余额字段 | 无法审计、并发下易错账、刷分难追溯 | 流水账本（ledger）为正本，余额物化 |
...
成熟燃币/虚拟货币体系的共识：**用只增不改（append-only）的流水账本记每一笔变动，余额是账本的物化（materialized）结果**，而不是直接 `UPDATE balance = balance + n`。
...
| **游客** | **零燃币、不可赚不可花** | 从入口就拦截，避免刷注册套利 |

### 3.3 内容门槛（gated content）的三种形态

| 形态 | 说明 | 适用 |
|---|---|---|
| 全文燃币墙 | 整篇/整个资源需燃币解锁 | 高价值资源、下载类 |
| 部分预览 + 解锁 | 前段免费，正文需燃币 | 长文、教程 |
| 阶梯角色门槛 | 按角色（游客/会员/信任）放行，燃币只是其一 | 与 `site_users.role` 配合 |
...
> **合规提示（需自行核实最新法规）**：中国《互联网跟帖评论服务管理规定》对跟帖评论服务存在「**后台实名、前台自愿**」一类要求。游客评论是否需要落实、怎样落实，属于法规与运营判断；技术方案不作法律结论，相关问题列入「未能验证清单」。
...
**解锁判定（服务端）**：游客 → 401「登录后用燃币解锁」；会员已在 `resource_unlocks` → 放行；余额≥cost → 扣分（插 ledger 负数 + upsert 余额 + 插 unlock，建议一个事务/批处理）→ 放行；不足 → 提示「再赚 X 分」。
...
- **观望**：设备指纹强反刷（等真出现刷分再加）；燃币提现/换现金（涉及会计与合规，门户站不必要）；跨设备游客身份（价值低于成本）。

### 4.6 下一步（建议分两期）

1. **一期·游客评论**：新增 `tuaran_guest` 签名 Cookie + `guest_bindings` 表；改造 `comments` POST 接受游客（更严限流 + 首条审核可选）；登录入口加归并步骤。
2. **二期·燃币解锁**：上四张燃币表 + 赚取入口（注册/签到/评论）+ 资源解锁 API + 前端燃币墙组件 + `/admin` 燃币与门槛配置页。
...
| 产品 / 体系 | 游客身份做法 | 绑定 | 燃币/门槛 |
|---|---|---|---|
| Disqus | 游客填昵称+邮箱即评 | 按邮箱归并 | 无燃币，靠声誉/审核 |
| 知乎 / 掘金类社区 | 需登录为主，匿名能力有限 | 账号体系内 | 盐值/矿石类虚拟燃币 + 解锁/兑换 |
| V2EX | 铜币/铸币燃币 | 账号内 | 发帖/回复得币，置顶等消费 |
| Firebase / Supabase | 匿名 Auth 拿临时 uid | 升级绑定，uid 不变 | 不含燃币，仅身份层 |

**可借鉴的结构性共识**：身份层（匿名→绑定）与燃币层（账本→门槛）解耦；游客只在身份层存在，燃币层从注册才开始。
...
| 燃币初值与上限的运营效果 | 表中为建议初值，未经真实数据校准 | 上线后按行为数据调参 |

---

## 七、结语与信息来源

**一种设计视角**：这件事的复杂度不在「燃币」本身，而在两层解耦——把「匿名→注册」放在身份层、把「赚取→消费」放在燃币层，各自独立演进。对 2aran.com 的幸运之处是身份层的难点（签名 Cookie、限流、用户目录）已经建好，新增的主要是燃币层的四张表和解锁判定。先做游客评论、再做燃币解锁，能让每一期都独立可用、风险可控。
