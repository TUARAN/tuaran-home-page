...
把四者并排打星，容易制造「AgentScope 全五星、LangChain 全一星」的错觉。但 **LangChain 的「一星多 Agent」是定位**——它本就是单 Agent 工具库；拿运行时的标准去要求一个库，等于拿卡车的载重去苛责一辆轿车。更有用的读法是：**先确定你站在哪一层**。
...
| 层 | 职责 | 该层常见形态 |
|---|---|---|
| 工具库层 | 把模型、工具、提示、记忆封装成可调用件 | 单 Agent 库（如 LangChain 类） |
| 编排层 | 定义多步 / 多 Agent 的流程、状态、分支与恢复 | 图 / 状态机引擎（如本文 LangGraph 类） |
| 运行时层 | 沙箱、权限、多租户、可观测、部署 | 企业级运行时（如 AgentScope 2.0 类，及各云厂商 Agent 平台） |
...
**这一节的核心判断**：两者 Skill 的分野是 **「工程化业务能力包（沙箱 / 权限 / 版本 / 团队共享 / 多语言执行）」 对 「自然语言驱动的本地自动化指令」**——前者服务多 Agent / 多租户的生产系统，后者服务单人本机的开箱即用。
...
- 主要资料来自各项目官方文档与公开仓库；版本和维护状态以官方最新信息为准，星级只表示相对侧重，不是绝对评分，也不构成选型承诺或商业建议。资料截至 2026-06-26。
