---
title: Agent Loop 深度调研：它和 Prompt、RAG、LangChain 是什么关系，为什么火在现在
category: topics
topic_type: tech
date: 2026-07-02
time: 23:08
tags: [Agent, Agent Loop, Prompt Engineering, RAG, LangChain, MCP, Tool Use, ReAct, METR, 技术调研]
summary: 把 agent loop 放回坐标系：prompt 是单次调用的输入技艺，RAG 是喂知识的检索管道，LangChain 是工程封装，agent loop 改的是调用结构本身——把「下一步做什么」的决定权交给模型。它 2022 年就被发明，火在 2024-2026 是因为模型单步准确率过了阈值、工具接口被 MCP 标准化、编码场景先赚到了验证。
tldr: Agent loop 的最小形态只是一个 while 循环：模型调工具、看结果、自己决定何时停。结构 2022 年（ReAct）就有，2023 年 AutoGPT 那轮炒作死于模型撑不住多步；这轮能成立，是 METR 口径下模型可完成任务时长翻倍周期从 7 个月加速到 4 个月、MCP 把工具接入变成标准件、编码代理先跑通了商业验证。Prompt、RAG、LangChain 都没死，只是从架构中心降级成了 loop 里的组件。
...
**Agent loop 是调用结构的改变，不是又一个新组件。** 单次 prompt 是「问一次、答一次」，agent loop 是 `while(未完成) { 模型决策 → 调工具 → 把结果喂回去 }`，把「下一步做什么、什么时候算完成」的判定权从开发者代码移交给模型。

要点：

1. **它和 prompt、RAG、LangChain 不在一个层上**。prompt 回答「一次调用里怎么问」，RAG 回答「调用前喂什么知识」，LangChain 回答「这些东西怎么封装成工程」，agent loop 回答「谁来决定下一步」。四者是配套关系，经常被并排比较是个误会。
2. **结构本身是旧的**。学术源头 ReAct 发表于 2022 年 10 月，AutoGPT 2023 年春就把这个循环跑给所有人看过，然后失败了——当时模型的单步准确率撑不起多步循环。
3. **这轮火起来靠三件事同时到位**：模型能力过阈值（METR 测得可完成任务时长的翻倍周期从 7 个月加速到约 4 个月）、工具接口标准化（MCP 从 2024-11 发布到 2026-03 月下载量近亿）、编码场景先完成商业验证（Claude Code、Cursor 类产品）。
4. **生态在围绕它重新站位**：LangChain 1.0（2025-10）把框架核心从 chain 重构为 agent loop（`create_agent`）；RAG 从 2023 年的架构中心降级为 loop 里的一个检索工具。
...
三个要素：一组工具定义、一个循环、一个停止条件（模型不再调工具，或达到步数/预算上限的兜底）。代码量小，难点全在别处：任务边界怎么划、失败时如何诊断是第几步偏航、副作用如何兜底。站内 [从 chat 任务到 agent loop：Anthropic 一线工程实践](/articles/research/topics/chat-to-agent-loop-anthropic-practice) 专门写过这条升级路径，本文不重复。
...
| 2022-10 | LangChain 开源 | 把 prompt / 检索 / chain 封装成框架，成为上一代 LLM 应用的默认脚手架 |
...
### 2.3 概念坐标系：五个词各自回答什么问题

| 概念 | 回答的问题 | 控制权在谁 | 典型失败模式 | 现在的位置 |
|---|---|---|---|---|
| **Prompt 工程** | 一次调用里怎么问 | 开发者 | 答非所问、格式漂移 | 没消失，转型为 system prompt 与工具描述设计（见 2.4） |
| **RAG** | 调用前喂什么知识 | 开发者（检索管道固定） | 检索不中、上下文塞垃圾 | 从架构中心降为 loop 里的一个检索工具（见 2.5） |
| **Workflow 编排** | 多次调用怎么串 | 开发者（路径预编排） | 分支覆盖不全、僵硬 | 仍是多数生产系统的主体，Anthropic 官方也建议优先用 |
| **Agent loop** | 谁决定下一步 | 模型（代码只兜底） | 跑偏、死循环、副作用失控 | 本文主角，2024-2026 的增量 |
| **Multi-agent** | 多个 loop 怎么分工 | 混合 | 通信开销、错误级联 | 大多数场景过度工程，见站内[框架横评](/articles/research/topics/multi-agent-frameworks-comparison) |

### 2.4 和 prompt 对比：变的是结构，不变的是输入质量

单次 prompt 的天花板是结构性的：

- **无反馈**：模型看不到自己输出的执行结果，错了没机会纠；
- **无中间状态**：多步任务只能靠开发者手动把上一轮结果粘进下一轮；
- **上下文一次性塞满**：所有可能用到的资料都得预先塞进去，塞多了淹没重点。

Agent loop 把这三条都翻过来：每步工具结果自动回喂（有反馈）、messages 数组就是状态（有记忆）、模型按需调工具取资料（按需加载上下文）。

但 prompt 工程没有被取代，工作对象换了。原来优化的是「用户问题怎么措辞」，现在优化的是三样：system prompt（agent 的行为边界与偏好）、工具描述（模型靠它决定调不调、怎么调，一句含糊的工具描述能让整个 loop 报废）、错误信息的回喂格式（决定模型能否自我纠偏）。Anthropic 官方文档把这块叫 [tool 设计](https://www.anthropic.com/engineering/writing-tools-for-agents)，篇幅比传统 prompt 技巧还多。

### 2.5 和 RAG 的关系：从架构中心到一个工具

2023-2024 年，「LLM 应用」四个字接近等于 RAG：切块、向量化、检索、拼 prompt，管道由开发者写死，检索发生在模型开口前。

Agent loop 改变了检索发生的位置。检索变成 loop 里的一个工具（`search` / `read_file` / `query_db`），什么时候查、查什么、查几轮由模型决定——查一次觉得不够可以换关键词再查。这个模式被叫做 agentic retrieval / agentic RAG。外部能观察到的变化：Claude Code 这类编码代理没有向量库，靠 grep + 读文件的多轮循环做代码检索；LangChain 1.0 文档里 retrieval 一章也已经把「retrieval as a tool」列为推荐模式。

RAG 没死。预检索管道在延迟敏感（客服首响）、语料固定（文档问答）的场景仍然更便宜更稳；变化在于它从「LLM 应用的默认架构」降级成「agent 众多工具中的一个」。

### 2.6 和 LangChain 的关系：框架自己的转身就是风向标

LangChain 是观察这轮变化最好的样本，因为它两头都做过：

- **2022-2024**：核心抽象是 chain——开发者预先把「prompt 模板 → 模型 → 解析器 → 下一步」串成固定管道。这是 workflow 思路。
- **2025-10 的 1.0 版本**：官方博客明确写「refocus on the core agent loop」，核心 API 换成 [`create_agent`](https://docs.langchain.com/oss/python/langchain/agents)——传 model、tools、system_prompt 三个参数，返回一个跑在 LangGraph 运行时上的 ReAct 循环。原来那堆 chain 抽象大量废弃。

一个靠 chain 起家的框架把 1.0 版本押在 agent loop 上，比任何布道文章都能说明生态共识走到了哪。四个主流框架（LangChain / LangGraph / AgentScope / AutoGen）的横向对比站内已有[单独一篇](/articles/research/topics/multi-agent-frameworks-comparison)，此处不展开。
...
Loop 的价值随可用工具数增长，而 2024 年前每个工具都要为每个模型单独写集成。MCP（2024-11）把这变成标准件：工具方实现一次 MCP server，任何支持 MCP 的模型/客户端都能用。采用曲线上文时间线已列：OpenAI 2025-03 跟进，2025-12 进入 Linux 基金会中立治理，ChatGPT / Cursor / Gemini / Copilot / VS Code 均已接入。工具生态从「大厂专属」变成「个人开发者周末能写一个」，loop 里可调用的东西多了两个数量级。
...
- **LangChain 生产占有率**：无权威统计，「默认脚手架」的说法基于 GitHub star / 招聘 JD 等软信号。
