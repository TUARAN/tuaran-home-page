...
summary: 以短信式硬件桥接台为实例，核对 WorkBuddy 的 OAuth、云端任务与 ACP 双通道流程，并用宽表说明 ACP 负责 Agent 会话、MCP 负责工具和数据接入的边界。
tldr: 当前桥接台确实使用 WorkBuddy 官方公开的 ACP 通道：先通过 Open API 获取任务的 link 与 token，再用 GET SSE 接收消息、POST JSON-RPC 发送 initialize、session/load 和 session/prompt。它是面向 WorkBuddy 的 ACP v1 最小客户端，尚未覆盖取消、自动重连、Schema 校验、ACP token 更新和完整权限请求分类。开放平台会按实际交付形态审核产品分类；只有桌面模拟器、平台服务和未来硬件设想时，不能作为硬件应用提交。硬件作为用户消息入口时优先用 ACP；硬件作为可被 Agent 调用的传感器或执行器时更适合做 MCP Server。
topic_type: tech
...
1. **当前实现是真实的 WorkBuddy ACP 接入。** 接收通道使用 GET SSE，发送通道使用 POST JSON-RPC，两条通道通过 `Acp-Connection-Id` 关联。
...
[WorkBuddy 第三方应用文档](https://open.workbuddy.cn/docs/third-party-app)把开放平台分成应用注册、OAuth 授权、Scope 管理和 Open API 调用。当前桥接台只申请两个权限：
...
昵称、头像、手机号、本地助理状态和积分都不参与当前流程，因此不申请 `user.profile.readable`、`user.contact.readable`、`user.localassistant.*` 或 `user.credit.exchange`。
...
| 实际要交付的产品 | 更匹配的入口 | 提交前应具备什么 | 与当前代码的差距 |
|---|---|---|---|
...
| 面向某个行业的完整 AI 工作台 | Buddy 应用 | 工作模式、系统提示、Skill、连接器和场景配置 | 需要重新定义垂直行业产品，当前桥接台不属于这一形态 |
| 独立桌面 ACP Client，用来控制 WorkBuddy 会话 | 现有公开入口没有完全对应项 | 需向平台确认是否开放普通第三方客户端注册 | 当前产品的准确分类 |

当前目标如果包含真实短信硬件，继续走“硬件接入”，先完成可展示的原型，再选择下拉框中最接近“开发板/开发套件”“通信终端”或“其他智能硬件”的类型；具体名称以平台实际选项为准。当前目标如果只有 macOS 软件，应停止按硬件应用反复提交，改造为 MCP 连接器，或向平台确认普通第三方 ACP 客户端能否取得 Open API 凭据。
...
当前实现会生成一次性 `state`，有效期 10 分钟，回调时验证并立即删除。Open API access token 接近过期时使用 refresh token 更新；token 文件使用 `0600` 权限保存。Electron 设置中的 Client Secret 通过系统安全存储加密后落盘。

这里仍有一条公开分发边界。[WorkBuddy Open API 文档](https://open.workbuddy.cn/docs/openapi)要求 Client Secret 不得暴露在前端或客户端代码中。内部联调可以利用本机安全存储降低误泄露风险；面向外部用户分发时，桌面应用无法成为真正的机密客户端，应该改成 PKCE 公共客户端模式，或让受控服务端完成 code 交换。开放平台是否支持不带 Secret 的原生应用模式，当前公开资料没有给出明确承诺。
...
| 多内容类型 | ACP 支持多种 ContentBlock | 当前只发送文本 | 部分覆盖 |
| 请求分类 | 区分权限、提问和其它服务端请求 | 当前统一映射为 permission | 待补 |
| WorkBuddy 扩展 | 可收到 `_codebuddy.ai/*` | 可显示原始消息，未结构化处理 | 可忽略、可增强 |
| 通用互操作 | 可连接任意 ACP Agent | 绑定 WorkBuddy 的鉴权和 HTTP Profile | 不属于通用客户端 |

因此，当前实现可以写成“基于 WorkBuddy ACP v1 的本地硬件桥接客户端”。“完整实现 ACP 标准”会超过现有代码所能证明的范围。
...
| 典型 Client | IDE、聊天 UI、终端、移动端、硬件交互界面 | AI Host 内部的 MCP Client | 当前桥接进程属于前一类 |
...
| 一轮结束 | prompt response + `stopReason` | Tool result 或异步 Task result | 当前按 prompt response 判定完成 |
...
| 文件能力 | Client 可向 Agent提供受限文件读写 | 文件系统通常作为 MCP Server | 当前 ACP Client主动声明不提供 |
| 终端能力 | Client 可向 Agent提供 Terminal | Shell 可以包装为 MCP Tool | 当前两边都不开放更安全 |
...
| 最适合 | 构建完整 Agent 前端或远程控制入口 | 为 Agent 增加可发现、可调用能力 | 当前入口选 ACP，设备能力选 MCP |
| 当前实例 | 把一条短信送入 WorkBuddy 会话 | 让 WorkBuddy 调用短信网关或传感器 | 两层可以组合 |
...
| 15. 断线恢复 | 重新获取 task token 并重建 ACP | 会话可继续追问 | 当前版本尚未自动实现 |
...
| P1 | 持久化幂等记录 | 当前内存去重在重启后失效 | 重启后重复 eventId 仍不重复执行 |
...
- [MCP 2025-11-25 Schema](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/schema/2025-11-25/schema.json)：当前稳定版协议的数据结构与方法。

### 资料边界

- 协议与平台事实以 2026 年 9 月 8 日可访问的官方文档为准。
- “当前实现”来自 ACP 短信桥接台本地代码、测试和运行结果，属于项目实测证据。
