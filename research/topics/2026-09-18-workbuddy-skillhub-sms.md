...
date: 2026-09-18
time: "12:30"
updated: 2026-09-18T15:30:00+08:00
...
- 899 条分词结果里，有多少会被 WorkBuddy 技能搜索实际展示、如何去重和排序。
- 任何技能的真实发送条数、送达率、投诉率、金额消耗。
- 创蓝两份 skill 是否同源 fork、是否得到创蓝官方授权。
- 中移「短信通知」在 WorkBuddy 里的授权界面、号码绑定方式和失败回退。摘要是产品主张，不是实测记录。
- `anti-fraud` 同 slug 两份如何去重，428,146 次下载有多少来自 WorkBuddy。
- 阿里云 `sdk-team`、腾讯云 `tencent-adm` 是否为云厂商官方账号。
...
- WorkBuddy 推荐算法、已安装明细、技能真实调用次数。
- 创蓝、阿里云、腾讯云、中移侧的计费与送达日志。
- ClawHub 与 SkillHub 下载事件是否去重。

**推断**

- 「搜索热度不等于发送能力」来自技能功能与下载量对照，不是平台官方解读。
- 「发给自己才是国内官方愿意上架的形态」是把中移摘要和既有监管结构放在一起的外部观察。
- installs / downloads 比值只说明两个字段量级不同，不能换算独立用户。

**资料截至** 2026-09-18 15:30（北京时间）。SkillHub 的 `updated_at` 几乎每条都在当日被 pipeline 刷新，下载量会继续变；引用时请以当时接口为准。
