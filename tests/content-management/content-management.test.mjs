...
test('admin exposes one content management entry and one unified list', () => {
  assert.match(centerSource, /getWorkspaceHubProps\('\/admin\/content'\)/)
  ...
  assert.match(consoleSource, /后台文章编辑器/)
  ...
  assert.doesNotMatch(consoleSource, /panel === 'import'/)
  assert.doesNotMatch(consoleSource, /ResearchImportConsole/)
  ...
  assert.match(consoleSource, /href="\/admin\/articles\/research-import"/)
