...
test('fixed pool images persist on the draft, retries reuse them, and list pagination retains history', async (t) => {
  const fixture = await assetFixture(t)
  const { db, bucket, objects } = fixture
  const key = await seedPool(fixture, 'greeting')
...
test('pool, storage, authentication and pause failures cannot send text-only posts', async (t) => {
  ...
  sqlite.exec('UPDATE x_image_pool SET enabled = 0')
  assert.equal((await invoke()).status, 502)
  sqlite.exec('UPDATE x_image_pool SET enabled = 1')
...
test('image strategy always uses the same-theme fixed pool and never invokes generation', async (t) => {
  for (const value of [0, 0.499999, 0.5, 0.999999]) {
    ...
    const image = await assetLibrary.prepareXImage({ ...fixture, asset, random: () => value,
      ai: { run: async () => { images++; return { image: PNG } } },
...
test('pool-only mode does not request an image brief or call the image model', async (t) => {
  const fixture = await assetFixture(t)
  const key = await seedPool(fixture)
  const asset = await assetLibrary.claimXAsset(fixture.db, { date: '2026-08-28', slot: 'morning', contentType: 'greeting' })
  await assetLibrary.prepareXImage({ ...fixture, asset, random: () => 0,
    createPrompt: async () => assert.fail('fixed templates do not need a prompt'),
    ai: { run: async () => assert.fail('fixed templates do not invoke the image model') },
  })
  assert.equal(asset.row.object_key, key)
  assert.equal(asset.row.asset_source, 'pool')
})

test('an unpublished generated draft is replaced by a fixed pool image on retry', async (t) => {
  const fixture = await assetFixture(t)
  const key = await seedPool(fixture)
  const asset = await assetLibrary.claimXAsset(fixture.db, { date: '2026-08-28', slot: 'morning', contentType: 'greeting' })
  fixture.objects.set('images/x-posts/old-generated.png', Uint8Array.from(Buffer.from(PNG, 'base64')))
  await assetLibrary.updateXAsset(fixture.db, asset, {
    object_key: 'images/x-posts/old-generated.png', mime_type: 'image/png', asset_source: 'generated', status: 'failed',
  })
  await assetLibrary.prepareXImage({ ...fixture, asset,
    ai: { run: async () => assert.fail('retired image generator must not run') },
  })
  assert.equal(asset.row.object_key, key)
  assert.equal(asset.row.asset_source, 'pool')
})
...
test('fixed pool selection ignores image-provider failures and reuses the chosen image on retry', async (t) => {
  ...
  await seedPool(fixture, 'crypto-insight', 'other-theme')
  let attempts = 0
  fixture.env.AI.run = async () => { attempts++; throw new Error('provider must not be called') }
  assert.equal((await fixture.invoke()).status, 502)
  ...
  assert.equal(row.fallback_error, '')
...
test('fixed pool selection does not wait for an image provider', async (t) => {
  const fixture = await assetFixture(t)
  await seedPool(fixture)
  const asset = await assetLibrary.claimXAsset(fixture.db, { date: '2026-08-28', slot: 'morning', contentType: 'greeting' })
