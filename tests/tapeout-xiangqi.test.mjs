...
test('public dashboard keeps execution times and human authorization gates visible', () => {
  ...
  assert.match(source, /\/images\/tapeout-xiangqi-launch\.png/)
  assert.doesNotMatch(source, /站内试玩/)
  assert.doesNotMatch(source, /复制页面链接/)
  assert.doesNotMatch(source, /可直接发布的推文草稿/)
