  ...
  _sendOne(recipient, text) {
    return new Promise((resolve, reject) => {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN || !this.authed) {
        return reject(new Error('5G WS 未连接/未认证'));
      }
      let timer;
      let settled = false;
      const finish = (error) => {
      ...
      const payload = { type: 'send', apiKey: this.apiKey, content: text, to: recipient };
      const raw = JSON.stringify(payload);
      this._pendingSendReject = finish;
      timer = setTimeout(() => finish(new Error('WebSocket 写入超时，结果未知')), 10000);
      this.ws.send(raw, (err) => {
        if (err) return finish(err);
        this.log.info(`[maap] 📤 send textLength=${String(text).length}`);
        clearTimeout(timer);
