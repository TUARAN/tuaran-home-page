...
app_zip="$dist_dir/codex-model-switcher-macos-universal-v1.2.0.zip"
skill_zip="$dist_dir/install-codex-model-switcher-skill-v1.1.0.zip"
...
/bin/cp -R "$skill_dir" "$stage/install-codex-model-switcher"
/bin/cp -R "$build_app" "$stage/install-codex-model-switcher/assets/Codex 模型切换器.app"
/usr/bin/ditto -c -k --sequesterRsrc --keepParent "$stage/install-codex-model-switcher" "$skill_zip"
