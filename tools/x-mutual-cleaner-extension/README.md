...
3. 展开右下角面板里的「测试功能」，点击「关注候选」。
4. 扩展只处理右侧按钮是普通 `Follow` / `关注` 的账号；已经 Following、Follow back 或无法识别的行会跳过。
5. 当前口径：每轮最多 10 个，每个关注动作间隔 30 秒；完成 10 个后，本轮结束，建议至少暂停 15 分钟再继续；每日建议不超过 200 个。
6. 需要停止时，再点同一个按钮。
